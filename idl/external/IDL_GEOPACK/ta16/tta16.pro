...
pro tta16, pos_gsm_tvar, pdyn=pdyn, yimf=yimf, symh=symh, symc=symc, $
  ...
  if(var_names[0] eq '') then begin
    ...
    message, /continue, 'tta16 only accepts one position tplot variable as input'
    return
  end
  ...
  if n_elements(parmod) gt 0 then begin
    ; parmod: (1) Pdyn [nPa], (2) SymHc, (3) XIND, (4) IMF By [nT].
