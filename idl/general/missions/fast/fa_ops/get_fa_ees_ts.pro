...
FUNCTION Get_fa_ees_ts, t1, t2, NPTS=npts, START=st, EN=en,     $
                     PANF=pf, PANB=pb, ALL=all, IDXST=idxst, CALIB=calib

   raw = get_md_ts_from_sdt ('Eesa Survey', 2001, T1=t1, T2=t2,    $
   ...
   hdr_raw = get_ts_from_sdt ('Eesa_Survey_Packet_Hdr', 2001, NPTS=raw.npts, $
                              IDXST=raw.st_index)
   ...
   IF hdr_raw.valid EQ 0 THEN BEGIN
      print, '@(#)get_fa_ees_ts.pro	1.2: Error getting Header bytes.  Bytes will be nil.'
  ...
  ; delta-T
   ...
   dat.integ_t = (dat.end_time - dat.time)/dat.nenergy
   IF dat(0).nbins EQ 64 THEN   dat.integ_t = dat.integ_t/2.
