...
;
;
;	function to load FAST Teams survey O+ species data from the SDT
...
; CALLING SEQUENCE:
;
; 	data = get_fa_tso (time, [START=start | EN=en | ADVANCE=advance |
;				RETREAT=retreat])
;
; ARGUMENTS:
;
...
FUNCTION Get_fa_tso, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, 'get_fa_tso.pro: Badly formed data.  Getting next sample'
          first = 0
...
;     eff = REPLICATE (1., dat.dimsizes(0))
      mass = 0.165695                          ; mass eV/(km/sec)^2
      geomfactor = 0.0015
