...
FUNCTION Get_fa_tbp_3d, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      index = find_handle('tbp_time',tagname)
      IF index EQ 0 THEN BEGIN
	 dat = get_fa_tbp(0, /st)
	 dat = get_fa_tbp(dat.time, /ad)
	 t = dat.time
	 n = 0
      ENDIF ELSE BEGIN
...
;	      pix1 = ABS(15*side-pix)
;	      ang = pix1 + phibin*16
;	      data(*,(ABS(15*side-pix)+phibin*16)) = dat.data(*,pix)
;	      data(*,ang) = dat.data(*,pix)
;	   ENDFOR
	...
	ENDIF
      ...
      store_data, 'tbp_time', data = datstr

      magdir = ((dat0.magdir + magdir)/2) MOD 360.
      
;      theta = (theta + magdir) MOD 360.
