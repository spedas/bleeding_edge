...
FUNCTION Get_fa_ies, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, '@(#)get_fa_ies.pro	1.24: Badly formed data.  Getting next sample'
          first = 0

          dat = get_md_from_sdt ('Iesa Survey', 2001, TIME=inputTime,    $
                                 START = start, EN = en, $
      ...
      hdr_time = inputTime
      hdr_dat = get_fa_ies_hdr (hdr_time, INDEX=idx)
;      hdr_dat = get_fa_ies_hdr (hdr_time)
