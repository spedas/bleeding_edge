...
FUNCTION Get_fa_ses, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, '@(#)get_fa_ses.pro	1.21: Badly formed data.  Getting next sample'
          first = 0

          dat = get_md_from_sdt ('Sesa Survey', 2001, TIME=inputTime,    $
                                 START = start, EN = en, $
      ...
      hdr_dat = get_fa_ses_hdr (hdr_time, INDEX=idx)
;      hdr_dat = get_fa_ses_hdr (hdr_time)
...
;      IF keyword_set(calib) AND got_header_bytes THEN BEGIN
      IF keyword_set(calib) AND got_header_bytes AND $
        ( dat.dimsizes(0) LE 6 ) THEN BEGIN
...
;          geom = FLOAT(REPLICATE (1., dat.dimsizes(0), dat.dimsizes(1)))
      ...
      RETURN,  {data_name:	data_name, 				      $
                 ...
                 integ_t: 	6D*(dat.endTime - dat.time)/dat.dimsizes(0),  $
