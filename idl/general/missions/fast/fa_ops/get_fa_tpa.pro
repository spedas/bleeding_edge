...
FUNCTION Get_fa_tpa, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, 'get_fa_tpa.pro: Badly formed data.  Getting next sample'
          first = 0

          dat = get_md_from_sdt ('Tms_He_Pole_Data', 2001, TIME=inputTime,    $
      ...
      mass = 0.0208778                         ; mass eV/(km/sec)^2
      geomfactor = 0.0015

      ; get the header bytes for this time

      hdr_time = inputTime
      hdr_dat = get_fa_tpah_hdr (hdr_time)
