...
FUNCTION get_fa_mscrub_singerrs, time1, time2, NPTS=npts, START=st, EN=en,   $
		PANF=pf, PANB=pb, ALL=all

   sdat = get_ts_from_sdt ('MScrub_SumSingleErrs', 2001,      $
                          t1 = time1, t2 = time2, NPTS = npts,            $
                          START=st, EN=en, PANF=panf, PANB=panb, ALL=all)

   IF NOT sdat.valid THEN          RETURN, {data_name: 'Null', valid: 0}
   
   ; load up the data into IDL data structs

   sret = 							$
     {data_name:	sdat.data_name, 				$
       valid: 		sdat.valid,				$
       project_name:	'FAST', 			  	$
       units_name: 	'counts',			  	$
       values_procedure: 'VALUES_PROCEDURE', 			$
       start_time:	sdat.start_time,				$
       end_time:	sdat.end_time, 				$
       npts:		sdat.npts, 				$
       ncomp:		sdat.ncomp, 				$
       depth:		sdat.depth,				$
       time:		sdat.time,				$
       memerrors:	sdat.comp1, 				$
