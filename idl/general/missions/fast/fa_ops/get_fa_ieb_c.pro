...
FUNCTION Get_fa_ieb_c, inputTime, START=start, EN=en, ADVANCE=advance,  $
                     RETREAT=retreat, CALIB=calib,                    $
                     INDEX=index, FLUSH=flush, CACHE_SIZE=cache_size

   ; cache common
   common fa_ieb_common, fa_ieb_cache, fa_ieb_cache_hsk, fa_ieb_dat, $
                         fa_ieb_hdr_cache_hsk, fa_ieb_hdr_cache
   
   data_name = 'Iesa Burst'
   cidx = get_cache_md_from_sdt (data_name, 2001,                         $
                                 fa_ieb_cache_hsk, fa_ieb_cache,          $
   ...
   IF n_elements (fa_ieb_dat) EQ 0 THEN $
     fa_ieb_dat = make_fa_esa_struct(fa_ieb_cache.dimsizes)
   IF ( fa_ieb_dat.nenergy NE fa_ieb_cache.dimsizes(0) ) OR    $
      ( fa_ieb_dat.nbins NE fa_ieb_cache.dimsizes(1) ) THEN    $
     fa_ieb_dat = make_fa_esa_struct(fa_ieb_cache.dimsizes)
   
   ; setup times

   fa_ieb_dat.time = fa_ieb_cache.times(cidx)
   fa_ieb_dat.end_time = fa_ieb_cache.endtimes(cidx)
   inputTime = fa_ieb_dat.time
   
   ; get data values into correct dimensions here

   fa_ieb_dat.index = cidx + fa_ieb_cache.st_index
   fa_ieb_dat.data_name = data_name
   fa_ieb_dat.units_name = 'Counts'
   fa_ieb_dat.units_procedure = 'convert_esa_units2'
   min1 = fa_ieb_cache.min1(*,cidx)
   max1 = fa_ieb_cache.max1(*,cidx)
   min2 = fa_ieb_cache.min2(*,cidx)
   max2 = fa_ieb_cache.max2(*,cidx)
   IF (where(max2-min2 lt 0))(0) NE -1 THEN      $
     max2(where(max2-min2 lt 0))=max2(where(max2-min2 lt 0))+360
   fa_ieb_dat.theta = FLOAT (REPLICATE (1., fa_ieb_cache.dimsizes(0)) # $
                             ((max2+min2)/2.) mod 360.)
   fa_ieb_dat.energy = FLOAT (REBIN(([max1+min1])/2., fa_ieb_cache.dimsizes(0), $
                                    fa_ieb_cache.dimsizes(1)))
   fa_ieb_dat.denergy = FLOAT (REBIN ([max1 - min1], fa_ieb_cache.dimsizes(0), $
                                      fa_ieb_cache.dimsizes(1)))
   fa_ieb_dat.dtheta = FLOAT (REPLICATE (1., fa_ieb_cache.dimsizes(0)) # $
                              (max2 - min2))
   fa_ieb_dat.eff = REPLICATE (1., fa_ieb_cache.dimsizes(0))
   fa_ieb_dat.mass = 0.0104389
   fa_ieb_dat.geomfactor = 0.0002720

   ; get the header bytes for this index

   hdr_idx  = cidx+fa_ieb_cache.st_index
   hidx = get_cache_ts_1p_from_sdt ('Iesa_Burst_Packet_Hdr', 2001,	    $
                                    fa_ieb_hdr_cache_hsk, fa_ieb_hdr_cache, $
                                    INDEX=hdr_idx,			    $
   ...
   IF hidx LT 0 THEN BEGIN
      ...
      fa_ieb_dat.header_bytes = BYTARR(44)
      got_header_bytes = 0
      hdr_dat = {valid: 0, time: 0.D, bytes: bytarr(44)}
   ENDIF ELSE BEGIN
       hdr_dat = {valid: fa_ieb_hdr_cache.valid,      $
                  time:  fa_ieb_hdr_cache.time(hidx), $
                  bytes: fa_ieb_hdr_cache.comp1(*,hidx)}
      fa_ieb_dat.header_bytes = hdr_dat.bytes
      got_header_bytes = 1
   	...
   	nswp_spin=3072/(fa_ieb_cache.dimsizes(0)*2^(ishft((hdr_dat.bytes(4) and 48),-4)))
        fa_ieb_dat.theta = fa_ieb_dat.theta + ((180./(nswp_spin))* $ 
                                               (findgen(fa_ieb_cache.dimsizes(0))- $
                                                fa_ieb_cache.dimsizes(0)/2+.5)/ $
                                               (fa_ieb_cache.dimsizes(0)/2.) $
                                              ) # replicate(1.,fa_ieb_cache.dimsizes(1))
   ENDELSE
   ...
   IF keyword_set(calib) AND got_header_bytes THEN BEGIN
       ...
       IF geom(0) GE 0 AND $
         (n_elements(geom) EQ fa_ieb_cache.dimsizes(0) * $
          fa_ieb_cache.dimsizes(1)) THEN BEGIN
           fa_ieb_dat.geom = $
             reform(geom, fa_ieb_cache.dimsizes(0), fa_ieb_cache.dimsizes(1)) 
       ENDIF ELSE  BEGIN
           PRINT, 'Error getting geom factors for this packet.  Values will be 1.'
           fa_ieb_dat.geom = $
             FLOAT(REPLICATE (1., fa_ieb_cache.dimsizes(0), fa_ieb_cache.dimsizes(1)))
       ENDELSE
   ENDIF ELSE  BEGIN
       fa_ieb_dat.geom = $
         FLOAT(REPLICATE (1., fa_ieb_cache.dimsizes(0), fa_ieb_cache.dimsizes(1)))
   ...
   fa_ieb_dat.denergy(0,*) = 0.
   IF (2 AND hdr_dat.bytes(6)) NE 0 THEN  fa_ieb_dat.denergy(1,*) = 0.

   ; delta-T

   fa_ieb_dat.integ_t = (fa_ieb_cache.endTimes(cidx) - $
                         inputTime)/fa_ieb_cache.dimsizes(0)
   
   ; finally, the data
   
   fa_ieb_dat.data = fa_ieb_cache.values(*,*,cidx)
