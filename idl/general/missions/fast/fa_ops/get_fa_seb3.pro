...
FUNCTION Get_fa_seb3, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, '@(#)get_fa_seb3.pro	1.17: Badly formed data.  Getting next sample'
          first = 0

          dat = get_md_from_sdt ('Sesa 3 Burst', 2001, TIME=inputTime,    $
                                 START = start, EN = en, $
	...
	ENDIF ELSE BEGIN
      ...
      hdr_dat = get_fa_seb3_hdr (hdr_time, INDEX=idx)
;      hdr_dat = get_fa_seb3_hdr (hdr_time)
