...
FUNCTION Get_fa_seb2_ts, t1, t2, NPTS=npts, START=st, EN=en,     $
                     PANF=pf, PANB=pb, ALL=all, IDXST=idxst, CALIB=calib

   raw = get_md_ts_from_sdt ('Sesa 2 Burst', 2001, T1=t1, T2=t2,    $
   ...
   hdr_raw = get_ts_from_sdt ('Sesa2_Burst_Packet_Hdr', 2001, NPTS=raw.npts, $
                              IDXST=raw.st_index)
   ...
   IF hdr_raw.valid EQ 0 THEN BEGIN
      print, '@(#)get_fa_seb2_ts.pro	1.3: Error getting Header bytes.  Bytes will be nil.'
