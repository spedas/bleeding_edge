...
FUNCTION Get_fa_th_3d_eq, inputTime, START=start, EN=en, ADVANCE=advance,  $
                         RETREAT=retreat, CALIB = calib

   spec = 1				; H+ species
   eq_ang = [0, 1, 8, 9, 4, 5, 12, 13]
   magdir_offset = 0.0		;magnetic direction offset. need to change
   geomfactor = 0.0015
   pt_limits = [0., 360.]

   dat = get_fa_th_3d(inputTime, START = start, EN = en, ADVANCE = advance,  $
			 RETREAT=retreat)
   IF NOT dat.valid THEN       RETURN, {data_name: 'Null', valid: 0}

   nbins = 8		;In equator, there are 16 angle bins
   spin_fract = FLOAT(REPLICATE (0.25, dat.nenergy, nbins))
   units_procedure = 'convert_tms_units'
   geom = FLOAT(REPLICATE (2., dat.nenergy, nbins))
   domega = REPLICATE(0.601176, nbins)
   ...
   FOR i = 0, (nbins-1) DO  theta(*, i) = 22.5 + 45.*i
   dtheta =  FLOAT(REPLICATE (45., nbins))
...
;   ENDFOR
   ...
   data = dat.data(*, eq_ang, *) 

   last_hdr_time = dat.time - (dat.end_time - dat.time)/2.0
   hdr_mag = get_fa_th_hdr(last_hdr_time)	;call previous data header
