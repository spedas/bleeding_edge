...
;-
pro fa_fields_despin2, V58, V12, T1 = t1, T2 = t2, PHASE = phase, $
                       USE_V158 = use_v158, $
...
req_dqds = [req_dqds, 'FAST_E_0_S_GSE', 'FAST_E_0_S_GSM']
...
store_data,'V12', data=data
dlimit = {spec:0, ystyle:1, $
          ytitle:'V12(mV/m)'}
store_data,'V12', dlimit=dlimit

data = {x:time+start_time, y:v58_dat}
store_data,'V58', data=data
dlimit = {spec:0, ystyle:1, $
          ytitle:'V58(mV/m)'}
store_data,'V58', dlimit=dlimit

data = {x:time+start_time, y:bphase}
store_data,'BPHASE', data=data
dlimit = {spec:0, ystyle:1, $
          ytitle:'BPHASE'}
store_data,'BPHASE', dlimit=dlimit

; Q flag, 0 for ok data, 1 for Mag notch, 2 for S notch, 3 for both
qflag = bnotch58 & qflag[*] = 0
not_okbn = where(bnotch58 Eq 0 Or bnotch12 Eq 0, nnot_okbn)
not_oksn = where(snotch58 Eq 0 Or snotch12 Eq 0, nnot_oksn)
help, qflag
help, not_okbn
help, not_oksn

If(nnot_okbn Gt 0) Then qflag[not_okbn] = 1
If(nnot_oksn Gt 0) Then qflag[not_oksn] = qflag[not_oksn]+2
data = {x:time+start_time, y:qflag}
store_data, 'DATA_QUALITY', data=data
dlimit = {spec:0, ystyle:1, $
          ytitle: 'DATA_QUALITY'}
...
If(is_struct(e00)) Then Begin
   data = {x:e00.time, y:transpose([transpose(e00.comp1), $
                                    transpose(e00.comp2), $
                                    transpose(e00.comp3)])}
   store_data, 'FAST_E_0_S_GSE', data = data
   dlimit = {spec:0, ystyle:1, $
             ytitle:'E_0_S_GSE(mV/m)'}
   store_data, 'FAST_E_0_S_GSE', dlimit = dlimit
Endif Else dprint, 'No FAST_E_0_GSE data'

e01 = get_fa_fields('FAST_E_0_S_GSM')
If(is_struct(e01)) Then Begin
   data = {x:e01.time, y:transpose([transpose(e01.comp1), $
                                    transpose(e01.comp2), $
                                    transpose(e01.comp3)])}
   store_data, 'FAST_E_0_S_GSM', data = data
   dlimit = {spec:0, ystyle:1, $
             ytitle:'E_0_S_GSM(mV/m)'}
   store_data, 'FAST_E_0_S_GSM', dlimit = dlimit
