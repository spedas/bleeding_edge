...
FUNCTION Get_fa_ees_c, inputTime, START=start, EN=en, ADVANCE=advance,  $
                     RETREAT=retreat, CALIB=calib,                    $
                     INDEX=index, FLUSH=flush, CACHE_SIZE=cache_size

   ; cache common
   common fa_ees_common, fa_ees_cache, fa_ees_cache_hsk, fa_ees_dat, $
                         fa_ees_hdr_cache_hsk, fa_ees_hdr_cache
   
   data_name = 'Eesa Survey'
   cidx = get_cache_md_from_sdt (data_name, 2001,                         $
                                 fa_ees_cache_hsk, fa_ees_cache,          $
   ...
   IF n_elements (fa_ees_dat) EQ 0 THEN $
     fa_ees_dat = make_fa_esa_struct(fa_ees_cache.dimsizes)
   IF ( fa_ees_dat.nenergy NE fa_ees_cache.dimsizes(0) ) OR    $
      ( fa_ees_dat.nbins NE fa_ees_cache.dimsizes(1) ) THEN    $
     fa_ees_dat = make_fa_esa_struct(fa_ees_cache.dimsizes)
   
   ; setup times

   fa_ees_dat.time = fa_ees_cache.times(cidx)
   fa_ees_dat.end_time = fa_ees_cache.endtimes(cidx)
   inputTime = fa_ees_dat.time
   
   ; get data values into correct dimensions here

   fa_ees_dat.index = cidx + fa_ees_cache.st_index
   fa_ees_dat.data_name = data_name
   fa_ees_dat.units_name = 'Counts'
   fa_ees_dat.units_procedure = 'convert_esa_units2'
   min1 = fa_ees_cache.min1(*,cidx)
   max1 = fa_ees_cache.max1(*,cidx)
   min2 = fa_ees_cache.min2(*,cidx)
   max2 = fa_ees_cache.max2(*,cidx)
   IF (where(max2-min2 lt 0))(0) NE -1 THEN      $
     max2(where(max2-min2 lt 0))=max2(where(max2-min2 lt 0))+360
   fa_ees_dat.theta = FLOAT (REPLICATE (1., fa_ees_cache.dimsizes(0)) # $
                             ((max2+min2)/2.) mod 360.)
   fa_ees_dat.energy = FLOAT (REBIN(([max1+min1])/2., fa_ees_cache.dimsizes(0), $
                                    fa_ees_cache.dimsizes(1)))
   fa_ees_dat.denergy = FLOAT (REBIN ([max1 - min1], fa_ees_cache.dimsizes(0), $
                                      fa_ees_cache.dimsizes(1)))
   fa_ees_dat.dtheta = FLOAT (REPLICATE (1., fa_ees_cache.dimsizes(0)) # $
                              (max2 - min2))
   fa_ees_dat.eff = REPLICATE (1., fa_ees_cache.dimsizes(0))
   fa_ees_dat.mass = 5.68566e-6
   fa_ees_dat.geomfactor = .000147

   ; get the header bytes for this index

   hdr_idx  = cidx+fa_ees_cache.st_index
   hidx = get_cache_ts_1p_from_sdt ('Eesa_Survey_Packet_Hdr', 2001,	    $
                                    fa_ees_hdr_cache_hsk, fa_ees_hdr_cache, $
                                    INDEX=hdr_idx,			    $
   ...
   IF hidx LT 0 THEN BEGIN
      ...
      fa_ees_dat.header_bytes = BYTARR(44)
      got_header_bytes = 0
      hdr_dat = {valid: 0, time: 0.D, bytes: bytarr(44)}
   ENDIF ELSE BEGIN
       hdr_dat = {valid: fa_ees_hdr_cache.valid,      $
                  time:  fa_ees_hdr_cache.time(hidx), $
                  bytes: fa_ees_hdr_cache.comp1(*,hidx)}
      fa_ees_dat.header_bytes = hdr_dat.bytes
      got_header_bytes = 1
   	...
   	nswp_spin=3072/(fa_ees_cache.dimsizes(0)*2^(ishft((hdr_dat.bytes(4) and 48),-4)))
        fa_ees_dat.theta = fa_ees_dat.theta + ((180./(nswp_spin))* $ 
                                               (findgen(fa_ees_cache.dimsizes(0))- $
                                                fa_ees_cache.dimsizes(0)/2+.5)/ $
                                               (fa_ees_cache.dimsizes(0)/2.) $
                                              ) # replicate(1.,fa_ees_cache.dimsizes(1))
   ENDELSE
   ...
   IF keyword_set(calib) AND got_header_bytes THEN BEGIN
       ...
       IF geom(0) GE 0 AND $
         (n_elements(geom) EQ fa_ees_cache.dimsizes(0) * $
          fa_ees_cache.dimsizes(1)) THEN BEGIN
           fa_ees_dat.geom = $
             reform(geom, fa_ees_cache.dimsizes(0), fa_ees_cache.dimsizes(1)) 
       ENDIF ELSE  BEGIN
           PRINT, 'Error getting geom factors for this packet.  Values will be 1.'
           fa_ees_dat.geom = $
             FLOAT(REPLICATE (1., fa_ees_cache.dimsizes(0), fa_ees_cache.dimsizes(1)))
       ENDELSE
   ENDIF ELSE  BEGIN
       fa_ees_dat.geom = $
         FLOAT(REPLICATE (1., fa_ees_cache.dimsizes(0), fa_ees_cache.dimsizes(1)))
   ...
   fa_ees_dat.denergy(0,*) = 0.
   IF (2 AND hdr_dat.bytes(6)) NE 0 THEN  fa_ees_dat.denergy(1,*) = 0.

   ; delta-T

   fa_ees_dat.integ_t = (fa_ees_cache.endTimes(cidx) - $
                         inputTime)/fa_ees_cache.dimsizes(0)
   IF fa_ees_cache.dimsizes(1) EQ 64 THEN   $
     fa_ees_dat.integ_t = fa_ees_dat.integ_t/2.
   
   ; finally, the data
   
   fa_ees_dat.data = fa_ees_cache.values(*,*,cidx)
