...
FUNCTION Get_fa_eeb_c, inputTime, START=start, EN=en, ADVANCE=advance,  $
                     RETREAT=retreat, CALIB=calib,                    $
                     INDEX=index, FLUSH=flush, CACHE_SIZE=cache_size

   ; cache common
   common fa_eeb_common, fa_eeb_cache, fa_eeb_cache_hsk, fa_eeb_dat, $
                         fa_eeb_hdr_cache_hsk, fa_eeb_hdr_cache
   
   data_name = 'Eesa Burst'
   cidx = get_cache_md_from_sdt (data_name, 2001,                         $
                                 fa_eeb_cache_hsk, fa_eeb_cache,          $
   ...
   IF n_elements (fa_eeb_dat) EQ 0 THEN $
     fa_eeb_dat = make_fa_esa_struct(fa_eeb_cache.dimsizes)
   IF ( fa_eeb_dat.nenergy NE fa_eeb_cache.dimsizes(0) ) OR    $
      ( fa_eeb_dat.nbins NE fa_eeb_cache.dimsizes(1) ) THEN    $
     fa_eeb_dat = make_fa_esa_struct(fa_eeb_cache.dimsizes)
   
   ; setup times

   fa_eeb_dat.time = fa_eeb_cache.times(cidx)
   fa_eeb_dat.end_time = fa_eeb_cache.endtimes(cidx)
   inputTime = fa_eeb_dat.time
   
   ; get data values into correct dimensions here

   fa_eeb_dat.index = cidx + fa_eeb_cache.st_index
   fa_eeb_dat.data_name = data_name
   fa_eeb_dat.units_name = 'Counts'
   fa_eeb_dat.units_procedure = 'convert_esa_units2'
   min1 = fa_eeb_cache.min1(*,cidx)
   max1 = fa_eeb_cache.max1(*,cidx)
   min2 = fa_eeb_cache.min2(*,cidx)
   max2 = fa_eeb_cache.max2(*,cidx)
   IF (where(max2-min2 lt 0))(0) NE -1 THEN      $
     max2(where(max2-min2 lt 0))=max2(where(max2-min2 lt 0))+360
   fa_eeb_dat.theta = FLOAT (REPLICATE (1., fa_eeb_cache.dimsizes(0)) # $
                             ((max2+min2)/2.) mod 360.)
   fa_eeb_dat.energy = FLOAT (REBIN(([max1+min1])/2., fa_eeb_cache.dimsizes(0), $
                                    fa_eeb_cache.dimsizes(1)))
   fa_eeb_dat.denergy = FLOAT (REBIN ([max1 - min1], fa_eeb_cache.dimsizes(0), $
                                      fa_eeb_cache.dimsizes(1)))
   fa_eeb_dat.dtheta = FLOAT (REPLICATE (1., fa_eeb_cache.dimsizes(0)) # $
                              (max2 - min2))
   fa_eeb_dat.eff = REPLICATE (1., fa_eeb_cache.dimsizes(0))
   fa_eeb_dat.mass = 5.68566e-6
   fa_eeb_dat.geomfactor = .000147

   ; get the header bytes for this index

   hdr_idx  = cidx+fa_eeb_cache.st_index
   hidx = get_cache_ts_1p_from_sdt ('Eesa_Burst_Packet_Hdr', 2001,	    $
                                    fa_eeb_hdr_cache_hsk, fa_eeb_hdr_cache, $
                                    INDEX=hdr_idx,			    $
   ...
   IF hidx LT 0 THEN BEGIN
      ...
      fa_eeb_dat.header_bytes = BYTARR(44)
      got_header_bytes = 0
      hdr_dat = {valid: 0, time: 0.D, bytes: bytarr(44)}
   ENDIF ELSE BEGIN
       hdr_dat = {valid: fa_eeb_hdr_cache.valid,      $
                  time:  fa_eeb_hdr_cache.time(hidx), $
                  bytes: fa_eeb_hdr_cache.comp1(*,hidx)}
      fa_eeb_dat.header_bytes = hdr_dat.bytes
      got_header_bytes = 1
   	...
   	nswp_spin=3072/(fa_eeb_cache.dimsizes(0)*2^(ishft((hdr_dat.bytes(4) and 48),-4)))
        fa_eeb_dat.theta = fa_eeb_dat.theta + ((180./(nswp_spin))* $ 
                                               (findgen(fa_eeb_cache.dimsizes(0))- $
                                                fa_eeb_cache.dimsizes(0)/2+.5)/ $
                                               (fa_eeb_cache.dimsizes(0)/2.) $
                                              ) # replicate(1.,fa_eeb_cache.dimsizes(1))
   ENDELSE
   ...
   IF keyword_set(calib) AND got_header_bytes THEN BEGIN
       ...
       IF geom(0) GE 0 AND $
         (n_elements(geom) EQ fa_eeb_cache.dimsizes(0) * $
          fa_eeb_cache.dimsizes(1)) THEN BEGIN
           fa_eeb_dat.geom = $
             reform(geom, fa_eeb_cache.dimsizes(0), fa_eeb_cache.dimsizes(1)) 
       ENDIF ELSE  BEGIN
           PRINT, 'Error getting geom factors for this packet.  Values will be 1.'
           fa_eeb_dat.geom = $
             FLOAT(REPLICATE (1., fa_eeb_cache.dimsizes(0), fa_eeb_cache.dimsizes(1)))
       ENDELSE
   ENDIF ELSE  BEGIN
       fa_eeb_dat.geom = $
         FLOAT(REPLICATE (1., fa_eeb_cache.dimsizes(0), fa_eeb_cache.dimsizes(1)))
   ...
   fa_eeb_dat.denergy(0,*) = 0.
   IF (2 AND hdr_dat.bytes(6)) NE 0 THEN  fa_eeb_dat.denergy(1,*) = 0.

   ; delta-T

   fa_eeb_dat.integ_t = (fa_eeb_cache.endTimes(cidx) - $
                         inputTime)/fa_eeb_cache.dimsizes(0)
   
   ; finally, the data
   
   fa_eeb_dat.data = fa_eeb_cache.values(*,*,cidx)
