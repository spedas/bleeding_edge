...
#Run locally in working directory
cd /home/jimm/themis_sw/general/missions/fast/fa_ops/workdir

# setup SDT
source setup_sdt

# Check for Lock file, if it's there nothing happens, note that the esv,
# e4k, e16k and dsp processes all look for the same file, to avoid piling on
# processes
...
# Zero-pad orbit to 5 digits
    ...
    echo $orbit
#
# Check for file , e.g., fa_dsp_l2_19961211093933_01211_v01.cdf
#    set pattern = "/disks/data/fast/l2/e16k/*/fa_dsp_l2_*_${orbit}_v01.cdf"
...
# Write file for sdt_batch,
    rm -rf process_orbit_dsp.batch
    set line="BatchJob:  cdftest"
    echo $line > process_orbit_dsp.batch
    ...
    set line="NoPlots:"
    echo $line >> process_orbit_dsp.batch
    set line="IDL: run_process_dsp.pro"
    echo $line >> process_orbit_dsp.batch
    ...
    echo $line >> process_orbit_dsp.batch
    set line="PlotConfigurationFile: DSP_SVY_NoData"
    echo $line >> process_orbit_dsp.batch
#Now call sdt_batch using this file
    sdt_batch process_orbit_dsp.batch
#    endif
else
...
# Aggresively restart if the age is greater than 5 minutes
    if ($age > 300) then
        echo "$lfile is older than 5 minutes ($age seconds old). Killing all SDT processes"
# Cleanup SDT
        rm -f killsdt0_log
        idl killsdt_all.pro > killsdt0_log
        ps -ef | grep 'SDTRunIndex' | grep -v grep | awk '{print $2}' | xargs kill -9
