...
# Check for version 2 file, check to see if orbit is an integer
    set raw = `tr -d ' \t\r\n' < orbit.txt`
    if ("$raw" !~ [0-9]*) then
...
# Zero-pad orbit to 5 digits
    ...
    echo $orbit
# Check for file , e.g., fa_despun_esv_l2_19961211093933_01211_v02.cdf
    set pattern = "/disks/data/fast/l2/esv/*/fa_despun_esv_l2_*_${orbit}_v02.cdf"
...
# Run your IDL program (batch mode, no prompt)
	rm -f killsdt0_log
	idl killsdt_all.pro > killsdt0_log
# Really kill all SDTs, just in case
        ps -ef | grep 'SDTRunIndex' | grep -v grep | awk '{print $2}' | xargs kill -9
#clean out all CDF files
        rm -f *.cdf
#delete lock file
