...
# create lock file
    echo process_orbit_sfa > process_orbit.lock
...
# Cleanup SDT
        ...
        idl killsdt_all.pro > killsdt0_log
        ps -ef | grep 'SDTRunIndex' | grep -v grep | awk '{print $2}' | xargs kill -9
# If there is a single cdf file, then SDT has no data for this orbit, and will segmentation fault
# so check for a single cdf, and if true, then increment the orbit.txt file
