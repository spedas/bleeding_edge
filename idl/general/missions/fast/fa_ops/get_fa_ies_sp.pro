...
FUNCTION get_fa_ies_sp,time,START=start,EN=en,ADVANCE=advance,RETREAT=retreat,CALIB=calib,VARARR=vararr

;	Get first 2 samples

dat = get_fa_ies_c(time,START=start,EN=en,ADVANCE=advance,RETREAT=retreat,CALIB=calib)
	if (not dat.valid) then return,dat
dat2 = get_fa_ies_c(time,/ad,CALIB=calib)
...
while ((hdr.spin_phase gt 8) or ((hdr2.spin_phase gt 8) and ((64-hdr.nangle)*(96-hdr.nenergy) ne 0)) $
	...
	if niter gt 120 then begin
		print,'Bad data segment in sdt memory, exitting get_fa_ies_sp.pro'
		dat2.valid=2
		return,dat2
	endif
	niter=niter+1
...
while dat.valid eq 1 and navg lt avgs_spin do begin
	dat=sum3d_hdr(dat,get_fa_ies_c(time,/ad,CALIB=calib),HDR_IND=hdr_ind,/spin)
	navg = navg + 1
endwhile
