...
FUNCTION Get_fa_seb4_c, inputTime, START=start, EN=en, ADVANCE=advance,  $
                        RETREAT=retreat, CALIB=calib,                    $
                        INDEX=index, FLUSH=flush, CACHE_SIZE=cache_size

   ; cache common
   common fa_seb4_common, fa_seb4_cache, fa_seb4_cache_hsk, fa_seb4_dat, $
                          fa_seb4_hdr_cache_hsk, fa_seb4_hdr_cache

   data_name = 'Sesa 4 Burst'
   cidx = get_cache_md_from_sdt (data_name, 2001,                         $
                                 fa_seb4_cache_hsk, fa_seb4_cache,        $
   ...
   IF n_elements (fa_seb4_dat) EQ 0 THEN $
     fa_seb4_dat = make_fa_esa_struct(fa_seb4_cache.dimsizes)
   IF ( fa_seb4_dat.nenergy NE fa_seb4_cache.dimsizes(0) ) OR    $
      ( fa_seb4_dat.nbins NE fa_seb4_cache.dimsizes(1) ) THEN    $
     fa_seb4_dat = make_fa_esa_struct(fa_seb4_cache.dimsizes)
   
   ; setup times

   fa_seb4_dat.time = fa_seb4_cache.times(cidx)
   fa_seb4_dat.end_time = fa_seb4_cache.endtimes(cidx)
   inputTime = fa_seb4_dat.time
   
   ; get data values into correct dimensions here
   
   fa_seb4_dat.index = cidx + fa_seb4_cache.st_index
   fa_seb4_dat.data_name = data_name
   fa_seb4_dat.units_name = 'Counts'
   fa_seb4_dat.units_procedure = 'convert_esa_units2'
   min1 = fa_seb4_cache.min1(*,cidx)
   max1 = fa_seb4_cache.max1(*,cidx)
   min2 = fa_seb4_cache.min2(*,cidx)
   max2 = fa_seb4_cache.max2(*,cidx)
   IF (where(max2-min2 lt 0))(0) NE -1 THEN      $
     max2(where(max2-min2 lt 0))=max2(where(max2-min2 lt 0))+360
   fa_seb4_dat.theta = FLOAT (REPLICATE (1., fa_seb4_cache.dimsizes(0)) # $
                             ((max2+min2)/2.) mod 360.)
   fa_seb4_dat.energy = FLOAT (REBIN(([max1+min1])/2., fa_seb4_cache.dimsizes(0), $
                                    fa_seb4_cache.dimsizes(1)))
   fa_seb4_dat.denergy = FLOAT (REBIN ([max1 - min1], fa_seb4_cache.dimsizes(0), $
                                      fa_seb4_cache.dimsizes(1)))
   fa_seb4_dat.dtheta = FLOAT (REPLICATE (1., fa_seb4_cache.dimsizes(0)) # $
                              (max2 - min2))
   fa_seb4_dat.eff = REPLICATE (1., fa_seb4_cache.dimsizes(0))
   fa_seb4_dat.mass = 5.68566e-6
   fa_seb4_dat.geomfactor = 0.000625

   ; get the header bytes for this index

   hdr_idx  = cidx+fa_seb4_cache.st_index
   hidx = get_cache_ts_1p_from_sdt ('Sesa4_Burst_Packet_Hdr', 2001,	    $
                                    fa_seb4_hdr_cache_hsk, fa_seb4_hdr_cache, $
                                    INDEX=hdr_idx,			    $
   ...
   IF hidx LT 0 THEN BEGIN
      ...
      fa_seb4_dat.header_bytes = BYTARR(44)
      got_header_bytes = 0
      hdr_dat = {valid: 0, time: 0.D, bytes: bytarr(44)}
   ENDIF ELSE BEGIN
       hdr_dat = {valid: fa_seb4_hdr_cache.valid,      $
                  time:  fa_seb4_hdr_cache.time(hidx), $
                  bytes: fa_seb4_hdr_cache.comp1(*,hidx)}
      fa_seb4_dat.header_bytes = hdr_dat.bytes
      got_header_bytes = 1
   ENDELSE
   ...
   IF keyword_set(calib) AND got_header_bytes THEN BEGIN
       ...
       IF geom(0) GE 0 AND $
         (n_elements(geom) EQ fa_seb4_cache.dimsizes(0) * $
          fa_seb4_cache.dimsizes(1)) THEN BEGIN
           fa_seb4_dat.geom = $
             reform(geom, fa_seb4_cache.dimsizes(0), fa_seb4_cache.dimsizes(1)) 
       ENDIF ELSE  BEGIN
           PRINT, 'Error getting geom factors for this packet.  Values will be 1.'
           fa_seb4_dat.geom = $
             FLOAT(REPLICATE (1., fa_seb4_cache.dimsizes(0), fa_seb4_cache.dimsizes(1)))
       ENDELSE
   ENDIF ELSE  BEGIN
       fa_seb4_dat.geom = $
         FLOAT(REPLICATE (1., fa_seb4_cache.dimsizes(0), fa_seb4_cache.dimsizes(1)))
   ...
   fa_seb4_dat.integ_t = (fa_seb4_cache.endTimes(cidx) - $
                         inputTime)/fa_seb4_cache.dimsizes(0)

   ; finally, the data
   
   fa_seb4_dat.data = fa_seb4_cache.values(*,*,cidx)
