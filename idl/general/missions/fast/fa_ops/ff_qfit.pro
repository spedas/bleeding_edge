...
; 
;-
pro ff_qfit, data, phs, phsf=phsf, es=es, ec=ec, zero=zero, $
             do_sigma=do_sigma, sigma=sigma, $
             period=period, slide=slide, n_fitpts=n_fitpts, $
             out=out, max_err=max_err, bad_pts=bad_pts                 
...
IF npts LT 4 then BEGIN
    message, /info, "Need to give phase array."
    return
ENDIF

IF data_type(phs) NE 5 then BEGIN
    message, /info, 'Phs must be double array.'
    return
ENDIF
...
IF dat_type gt 5 or dat_type lt 1 then BEGIN
    message, /info, $
      'Data must be a byte, int, long, float, or double array.'
    return
ENDIF
IF n_elements(data) NE npts then BEGIN
    message, /info, 'Data and phase must have the same number of points.'
    return
ENDIF
...
fix_dat  = ff_interp(fix_phs,phs,data,delt=period/4.0, /spline)
fix_strt = 0l
...
WHILE fix_end LT n_phases DO BEGIN
    ...
    ; DO THE QUICK FIT!
    qfit(nfits) = total( fix_dat(fix_strt:fix_end) * fix_cos ) $
			* 2.d / n_fitpts 
...
IF keyword_set(do_sigma) or keyword_set(out) then BEGIN
    ...
    z_big   = ff_interp(fix_phs,phsf,zero,delt=period)
    c_big   = ff_interp(fix_phs,phsf,ec,delt=period)
    s_big   = ff_interp(fix_phs,phsf,es,delt=period)
    ...
    IF keyword_set(out) then BEGIN
        ...
        ; SET UP ARRAYS FOR SIGMA AND OUT
        z_big   = ff_interp(fix_phs,phsf,zero,delt=4.d*period)
        c_big   = ff_interp(fix_phs,phsf,ec,delt=4.d*period)
        s_big   = ff_interp(fix_phs,phsf,es,delt=4.d*period)
        fit_big = z_big + c_big * cos(fix_phs) + s_big * sin(fix_phs)

        fit_big = ff_interp(phs,fix_phs,fit_big, delt=period, /spline)
