;;  @(#)azonloc.pro	1.2 12/15/94   Fast orbit display program

function azonloc,mlt,q,poleward=pole,latitude=lat,south=south,degrees=deg

;  IDL function to calculate auroral zone position as a function of
;  magnetic local time (mlt: 0.0-24.0 hours), activity index Q (q: 0-6).
;  Returns corrected geomagnetic colatitude in radians.
;  Options:
;     To return poleward edge, set poleward.
...
apole=[[15.22,2.41,3.34,-0.85,1.01,0.32,0.90], $
       ...
       [16.71,0.37,2.90,0.63,1.59,0.60,1.00]]*!dtor
aequa=[[17.36,3.03,3.46,0.42,2.11,-0.25,1.13], $
       ...
       [23.18,4.85,3.34,-0.38,-0.62,-0.53,-0.16]]*!dtor

p=mlt/12.0*!pi
i=(fix(q)>0)<6
if(keyword_set(pole)) then $
  t=apole(0,i)+ $
