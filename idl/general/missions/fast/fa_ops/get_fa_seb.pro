...
FUNCTION Get_fa_seb, inputTime, START=start, EN=en, ADVANCE=advance,  $
   ...
   REPEAT BEGIN
       IF NOT first THEN       $
         print, 'get_fa_seb.pro: Badly formed data.  Getting next sample'
       first = 0

       dat = get_md_from_sdt ('Sesa_Cmb_Burst_Data', 2001, TIME=inputTime,    $
                              START = start, EN = en, $
   ...
   hdr_time = inputTime
   hdr_dat = get_fa_seb_hdr (hdr_time, INDEX=idx)

   IF hdr_dat.valid EQ 0 THEN BEGIN
       ; JBV - Since the combined Sesa burst does not have a packet hdr
       ; qty, comment out this print statement as it only clutters up
       ; the IDL window.
...
;       geom = FLOAT(REPLICATE (1., dat.dimsizes(0), dat.dimsizes(1)))
   ...
   RETURN,  {data_name:	data_name, 					      $
             ...
             integ_t: 		((dat.endTime - inputTime)/dat.dimsizes(0))*6,$
