...
FUNCTION Get_fa_eeb, inputTime, START=start, EN=en, ADVANCE=advance,  $
   ...
   REPEAT BEGIN
       IF NOT first THEN       $
         print, '@(#)get_fa_eeb.pro	1.20: Badly formed data.  Getting next sample'
       first = 0

       dat = get_md_from_sdt ('Eesa Burst', 2001, TIME=inputTime,    $
                              START = start, EN = en, $
   ...
   hdr_time = inputTime
   hdr_dat = get_fa_eeb_hdr (hdr_time, INDEX=idx)
;   hdr_dat = get_fa_eeb_hdr (hdr_time)
