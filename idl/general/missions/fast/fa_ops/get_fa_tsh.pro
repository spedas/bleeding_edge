...
;
;
;	function to load FAST Teams survey He+ species data from the SDT
...
; CALLING SEQUENCE:
;
; 	data = get_fa_tsh (time, [START=start | EN=en | ADVANCE=advance |
;				RETREAT=retreat])
;
; ARGUMENTS:
;
...
FUNCTION Get_fa_tsh, inputTime, START=start, EN=en, ADVANCE=advance,  $
      ...
      REPEAT BEGIN
          IF NOT first THEN       $
            print, 'get_fa_tsh.pro: Badly formed data.  Getting next sample'
          first = 0

          dat = get_md_from_sdt ('Tms_He_Survey_Data', 2001, TIME=inputTime,    $
...
;      eff = REPLICATE (1., dat.dimsizes(0))
      mass = 0.0414521                          ; mass eV/(km/sec)^2
      geomfactor = 0.0015

      ; get the header bytes for this time

      hdr_time = inputTime
      hdr_dat = get_fa_tsah_hdr (hdr_time)
   ...
   pac = header_bytes(11)			;post acceleration voltage
   eff = fa_ts_eff(energy, pac, spec, 0, 0, eff_version)
