...
FUNCTION get_ess,time,START=start,EN=en,ADVANCE=advance,RETREAT=retreat,VARARR=vararr

;	Get first 2 samples

dat = get_eesa_surv(time,START=start,EN=en,ADVANCE=advance,RETREAT=retreat)
	if (not dat.valid) then return,dat
dat2 = get_eesa_surv(time,/ad)
...
while ((hdr.spin_phase gt 2) or ((hdr2.spin_phase gt 2) and ((64-hdr.nangle)*(96-hdr.nenergy) ne 0)) $
	...
	if niter gt 120 then begin
		print,'Bad data segment in sdt memory, exitting get_ess.pro'
		dat2.valid=2
		return,dat2
	endif
	niter=niter+1
...
if (avgs_spin ne 2) then for a=0,avgs_spin-3 do dat=sum3d_hdr(dat,get_eesa_surv(time,/ad),HDR_IND=hdr_ind,/spin)
