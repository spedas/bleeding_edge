...
; and is a structure. jmm, 2022-06-14
  ...
  for i =0,N_ELEMENTS(tplot_vars)-1 do begin    
    ...
    if KEYWORD_SET(default_cdf_structure) then fa_tplot_add_cdf_structure,tname,tt2000=tt2000, $
    ...
    if ~undefined(x) then begin
      ...
      If(~keyword_set(add_tname_to_epoch)) Then Begin
      ...
      Endif Else Begin ;No generic epoch variables here
         EpochName = EpochVAR.NAME
         (*Epochvar.ATTRPTR).VAR_TYPE = 'support_data'
         EpochVARS = array_concat(EpochVAR,EpochVARS)
      Endelse
    ...
    if ~undefined(VAR) then begin
       attr = *VAR.ATTRPTR
       ppp = tag_exist(attr, 'VAR_TYPE')
       if ppp eq 0 then str_element, attr, 'VAR_TYPE', 'data', /add
       if STRCMP(attr.VAR_TYPE, 'undefined') then attr.VAR_TYPE = 'data' ;Change attributes for data variable variable
       qqq = tag_exist(attr, 'DISPLAY_TYPE')
       if qqq eq 0 then str_element, attr, 'DISPLAY_TYPE', 'time_series', /add
