...
pro fa_tplot_add_cdf_structure, tplot_vars, tt2000=tt2000, new=new, add_tname_to_epoch = add_tname_to_epoch
...
; === CDF_EPOCH ===
     if keyword_set(add_tname_to_epoch) then epochname = tplot_vars[i]+'_Epoch' $
     else epochname = 'Epoch'
     if ~undefined(x) then begin
        depend_0.name = epochname
        ...
        if KEYWORD_SET(tt2000) then begin
           ...
           str_element, attr, 'CATDESC', epochname+ ': Epoch variable', /add
           str_element, attr, 'LABLAXIS', 'UT', /add
        endif else begin        
           if size(x,/type) ne 5 then begin
              dprint,'Warning: x type is not double. You may expect error using tplot2cdf. CDF structure must be recreated. Use /new flag to create new CDF strucure',dlevel=1
              x = double(x)
           endif
...
;    endif
      ...
      ; Check if v is LABL_PTR_1
        ...
        ; if v is 2d
          ...
          ; vars.attrptr = ptr_new(CREATE_STRUCT('DEPEND_0','Epoch',*vars.attrptr)) ; one line addition to the attribute structure
              str_element, *depend_1.attrptr,'DEPEND_0',epochname,/add
           endif else begin
     ...
     if ~undefined(v2) then begin
      ...
      ; if v is 2d
        ...
        ; in this case first dimension of v is time (we don't check the actual number of records)
        ; vars.attrptr = ptr_new(CREATE_STRUCT('DEPEND_0','Epoch',*vars.attrptr)) ; one line addition to the attribute structure
           str_element, *depend_2.attrptr,'DEPEND_0',epochname,/add
        endif else begin
        ; if v is 1d then it does not change in time
           depend_2.recvary = 0b
        endelse
     ...
     if ~undefined(v3) then begin
      ...
      ; if v is 2d
        ...
        ; in this case first dimension of v is time (we don't check the actual number of records)
        ; vars.attrptr = ptr_new(CREATE_STRUCT('DEPEND_0','Epoch',*vars.attrptr)) ; one line addition to the attribute structure
           str_element, *depend_3.attrptr,'DEPEND_0',epochname,/add
        endif else begin
        ; if v is 1d then it does not change in time
           depend_3.recvary = 0b
        endelse
