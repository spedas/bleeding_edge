...
;datetime = dindgen(86400) + time_double('2012-09-24')
datetime = time_double(datetime)
...
;***
;Note that no end times are given for stacer deploys.
;***
...
;		 To Convert SPB total stroke to dipole tip-to-tip, double stroke length and
;		 add 1.82 m;
;    e.g. 20-m stroke = 41.8-m tip-to-tip dipole.
...
nelem = n_elements(datetime)
blength_A12 = replicate(0,nelem)
blength_A34 = replicate(0,nelem)
blength_B12 = replicate(0,nelem)
blength_B34 = replicate(0,nelem)
blength_A5 = replicate(0,nelem)
blength_A6 = replicate(0,nelem)
blength_B5 = replicate(0,nelem)
blength_B6 = replicate(0,nelem)


ndeploys = n_elements(deployendA12)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartA12[j])) and (datetime le time_double(deploystartA12[j+1])))
  if goo[0] ne -1 then blength_A12[goo] = 2*lengthA12[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendA12[ndeploys-1]))
if goo[0] ne -1 then blength_A12[goo] = 2*lengthA12[ndeploys-1] + 1.82


ndeploys = n_elements(deployendA34)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartA34[j])) and (datetime le time_double(deploystartA34[j+1])))
  if goo[0] ne -1 then blength_A34[goo] = 2*lengthA34[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendA34[ndeploys-1]))
if goo[0] ne -1 then blength_A34[goo] = 2*lengthA34[ndeploys-1] + 1.82



ndeploys = n_elements(deployendB12)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartB12[j])) and (datetime le time_double(deploystartB12[j+1])))
  if goo[0] ne -1 then blength_B12[goo] = 2*lengthB12[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendB12[ndeploys-1]))
if goo[0] ne -1 then blength_B12[goo] = 2*lengthB12[ndeploys-1] + 1.82


ndeploys = n_elements(deployendB34)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartB34[j])) and (datetime le time_double(deploystartB34[j+1])))
  if goo[0] ne -1 then blength_B34[goo] = 2*lengthB34[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendB34[ndeploys-1]))
if goo[0] ne -1 then blength_B34[goo] = 2*lengthB34[ndeploys-1] + 1.82




ndeploys = n_elements(deployendA5)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartA5[j])) and (datetime le time_double(deploystartA5[j+1])))
  if goo[0] ne -1 then blength_A5[goo] = 2*lengthA5[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendA5[ndeploys-1]))
if goo[0] ne -1 then blength_A5[goo] = 2*lengthA5[ndeploys-1] + 1.82

ndeploys = n_elements(deployendA6)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartA6[j])) and (datetime le time_double(deploystartA6[j+1])))
  if goo[0] ne -1 then blength_A6[goo] = 2*lengthA6[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendA6[ndeploys-1]))
if goo[0] ne -1 then blength_A6[goo] = 2*lengthA6[ndeploys-1] + 1.82


blength_A56 = blength_A5 + blength_A6
goo = where(blength_A56 ne 0.)
if goo[0] ne -1 then blength_A56[goo] = blength_A56[goo] + 1.2 + 0.76
...
ndeploys = n_elements(deployendB5)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartB5[j])) and (datetime le time_double(deploystartB5[j+1])))
  if goo[0] ne -1 then blength_B5[goo] = 2*lengthB5[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendB5[ndeploys-1]))
if goo[0] ne -1 then blength_B5[goo] = 2*lengthB5[ndeploys-1] + 1.82

ndeploys = n_elements(deployendB6)
for j=0,ndeploys-2 do begin
  goo = where((datetime gt time_double(deploystartB6[j])) and (datetime le time_double(deploystartB6[j+1])))
  if goo[0] ne -1 then blength_B6[goo] = 2*lengthB6[j] + 1.82
endfor
;length after final deploy
goo = where(datetime ge time_double(deployendB6[ndeploys-1]))
if goo[0] ne -1 then blength_B6[goo] = 2*lengthB6[ndeploys-1] + 1.82


blength_B56 = blength_B5 + blength_B6
goo = where(blength_B56 ne 0.)
if goo[0] ne -1 then blength_B56[goo] = blength_B56[goo] + 1.2 + 0.76
