...
; $LastChangedDate: 2026-08-27 11:11:51 -0700 (Thu, 27 Aug 2026) $
; $LastChangedRevision: 34817 $
...
pro st_swaves_load, type, all = all, files = files, trange = trange, $
  ...
  for i = 0, n_elements(probes) - 1 do begin
    ...
    if level eq 3 then begin
      ; https://spdf.gsfc.nasa.gov/pub/data/stereo/ahead/l3/waves/hfr-direction-finding/

      pref = 'st' + probe + '_swaves_' + type + '_'
      if probe eq 'a' then fullprobe = 'ahead' else fullprobe = 'behind'
      path = fullprobe + '/l3/waves/' + type + '-direction-finding/YYYY/st' + probe + '_l3_wav_' + $
