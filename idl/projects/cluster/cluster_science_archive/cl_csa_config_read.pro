; read_ascii template for cl_config_read 
FUNCTION cl_csa_config_read_template
...
FUNCTION cl_csa_config_read
  cfg = -1
  dir = cl_csa_config_filedir(/app_query); look for the config directory
  if(dir[0] ne '') then begin
    ...
    if file_test(filex) then begin;........ if dir found, look for the config file
      ttt = cl_csa_config_read_template()
