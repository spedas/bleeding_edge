...
-------------------------------------------------------------------------------
   ...
   Version 0.12 -- July 27, 2025 -- David Mitchell, SSL-UCB

      Added the MAVEN_MARS_ENR/-202931 dynamic frame.
        (ENR = East-North-Radial)

   Version 0.11 -- April 11, 2023 -- Matt Barnes, NAIF
                                     David Mitchell, SSL-UCB

      Updated MAVEN_STATIC/-202520 to the correct definition per [17].

   Version 0.10 -- March 17, 2023 -- Matt Barnes, NAIF
                                     Takuya Hara, SSL-UCB

      Added MAVEN_SUN_RTN/-202921 dynamic frame.

   Version 0.9 -- October 9, 2015 -- Boris Semenov, NAIF

      Added MAVEN_SEP*_S name/ID mappings. Re-arranged name/ID table
      in the comments to show synonyms on the right.

   Version 0.8 -- August 10, 2015 -- Boris Semenov, NAIF

      Added MAVEN_SEP*_ELEC/_ION name/ID mappings.

   Version 0.7 -- July 30, 2015 -- Boris Semenov, NAIF

      Added MAVEN_STATIC_FRONT/-202522 and MAVEN_STATIC_BACK/-202523
      name/ID mappings.

   Version 0.6 -- April 15, 2015 -- Boris Semenov, NAIF

      Added MAVEN_EUV-A/-202111, MAVEN_EUV-B/-202112, and
      MAVEN_EUV-C/-202113 name/ID mappings.

   Version 0.5 -- November 16, 2014 -- Boris Semenov, NAIF

      Incorporated SC -> APP_BP and APP_OG -> IUVS_BASE misalignments
      derived from October 2014 APP/IUVS calibrations from [16].

      Incorporated EUV and NGIMS alignments from [9] and [12].

      Added a note about pre-launch MAG alignments from [11] to the
      comments.

      Corrected deployed SWEA frame orientation diagram and description
      per [13]; added description of stowed SWEA orientation per [14];
      re-defined the SWEA frame to be a CK-based frame to support its
      orientation time dependency due to deployment.

      Updated IUVS_LIMB frame orientation based on [15].
...
-------------------------------------------------------------------------------
   ...
   9. EUV_alignment_final.xlsx

  10. IUVS_alignment_final.xlsx

  11. MAG_alignment_final.xlsx

  12. NGIMS_alignment_final.xlsx

  13. E-mail ``correction for MAVEN SWEA frame'' from David Mitchell,
      SSL, Berkeley, 07/09/14

  14. E-mail ``MAVEN SWEA - stowed boom transformation'' from David Mitchell,
      SSL, Berkeley, 08/01/14

  15. E-mail ``MAVEN frame kernel'' regarding IUVS limb FOR offset
      from Greg Holsclaw, LASP, 10/31/14

  16. E-mail ``MAVEN APPcal results'' from Liam Pisano, LMCO, 11/13/14

  17. E-mail ``Update to MAVEN STATIC frame'' from David Mitchell,
      SSL, Berkeley, 04/10/23
...
Sun MAVEN Radial-Tangential-Normal Frame

   Within the SPICE system, the Sun MAVEN Radial-Tangential-Normal
   frame is referred as MAVEN_SUN_RTN (ID -202921). In literature, this
   frame is referred as RTN, Orbit RTN Coordinate System or
   radial-transverse-normal.

   The Sun MAVEN Radial-Tangential-Normal frame is defined as follows:

      -  the position of MAVEN relative to the Sun is the
         primary vector: +X axis points from the Sun to
         MAVEN (antisunward direction);

      -  +Z axis is parallel to the Solar North;

      -  +Y axis completes the right-handed system;

      -  the origin of this frame is the center of mass of MAVEN.
   ...
   therefore, the ephemeris data required to compute the MAVEN-Sun state
   vector in the J2000 reference frame must be loaded before using this
   frame.

   The secondary vector is the Z axis of frame IAU_SUN; therefore, PCK data
   providing IAU_SUN orientation must be loaded for the frame to be
   computable.

   This frame is defined based on SPK data: different planetary
   ephemerides for the Sun, the Solar System Barycenter and MAVEN spacecraft
   ...
   MAVEN_SUN_RTN using the DE405 ephemeris and the MAVEN ephemeris
   version N.

   \begindata

      FRAME_MAVEN_SUN_RTN            = -202921
      FRAME_-202921_NAME             = 'MAVEN_SUN_RTN'
      FRAME_-202921_CLASS            =  5
      FRAME_-202921_CLASS_ID         = -202921
      FRAME_-202921_CENTER           = -202
      FRAME_-202921_RELATIVE         = 'J2000'
      FRAME_-202921_DEF_STYLE        = 'PARAMETERIZED'
      FRAME_-202921_FAMILY           = 'TWO-VECTOR'
      FRAME_-202921_PRI_AXIS         = 'X'
      FRAME_-202921_PRI_VECTOR_DEF   = 'OBSERVER_TARGET_POSITION'
      FRAME_-202921_PRI_OBSERVER     = 'SUN'
      FRAME_-202921_PRI_TARGET       = 'MAVEN'
      FRAME_-202921_PRI_ABCORR       = 'NONE'
      FRAME_-202921_SEC_AXIS         = 'Z'
      FRAME_-202921_SEC_VECTOR_DEF   = 'CONSTANT'
      FRAME_-202921_SEC_FRAME        = 'IAU_SUN'
      FRAME_-202921_SEC_SPEC         = 'RECTANGULAR'
      FRAME_-202921_SEC_VECTOR       = ( 0, 0, 1 )

  \begintext


Mars MAVEN East-North-Radial Frame

   Within the SPICE system, the Mars MAVEN East-North-Radial frame
   is referred as MAVEN_MARS_ENR (ID -202931). This is also referred
   to as the local azimuth-elevation frame.  The frame is centered on
   the MAVEN spacecraft, with azimuth measured in the local horizontal
   plane with respect to Mars and elevation is the angle out of the 
   horizontal plane.

   The Mars MAVEN East-North-Radial frame is defined as follows:

      -  the position of MAVEN relative to Mars is the primary
   ...
   The primary vector is defined as an 'observer-target position';
   therefore, the ephemeris data required to compute the MAVEN-Mars state
   vector in the J2000 reference frame must be loaded before using this
   frame.

   The secondary vector is the Z axis of frame IAU_MARS; therefore, PCK data
   providing IAU_MARS orientation must be loaded for the frame to be
   computable.

   This frame is defined based on SPK data: different planetary
   ephemerides for Mars and the MAVEN spacecraft will lead to different
   ...
   \begindata
      ...
      FRAME_MAVEN_MARS_ENR         =  -202931
      FRAME_-202931_NAME           = 'MAVEN_MARS_ENR'
      FRAME_-202931_CLASS          =  5
      FRAME_-202931_CLASS_ID       =  -202931
      FRAME_-202931_CENTER         =  -202
      FRAME_-202931_RELATIVE       = 'J2000'
      FRAME_-202931_DEF_STYLE      = 'PARAMETERIZED'
      FRAME_-202931_FAMILY         = 'TWO-VECTOR'
      FRAME_-202931_PRI_AXIS       = 'Z'
      FRAME_-202931_PRI_VECTOR_DEF = 'OBSERVER_TARGET_POSITION'
      FRAME_-202931_PRI_OBSERVER   = 'MARS'
      FRAME_-202931_PRI_TARGET     = 'MAVEN'
      FRAME_-202931_PRI_ABCORR     = 'NONE'
      FRAME_-202931_SEC_AXIS       = 'Y'
      FRAME_-202931_SEC_VECTOR_DEF = 'CONSTANT'
      FRAME_-202931_SEC_SPEC       = 'RECTANGULAR'
      FRAME_-202931_SEC_FRAME      = 'IAU_MARS'
      FRAME_-202931_SEC_VECTOR     = ( 0, 0, 1 )
...
EUV Frames
   ...
   Reference [9] provides the following "Final Flight Knowledge Data,
   Measured DCM, EUV to SMRF (EUV in Columns relative to SMRF)":

          0.9999986983942670   0.0015903565855873   0.0002719847482266
         -0.0015907341441573   0.9999977636853660   0.0013936244652375
         -0.0002697677801374  -0.0013940553067130   0.9999989919170660

   This DCM is incorporated into the MAVEN_EUV frame definition
   starting with the FK version 0.5.
   ...
   \begindata
      ...
      TKFRAME_-202110_SPEC            = 'MATRIX'
      TKFRAME_-202110_RELATIVE        = 'MAVEN_SPACECRAFT'
      TKFRAME_-202110_MATRIX          = (

         0.9999986983942670  -0.0015907341441573  -0.0002697677801374
         0.0015903565855873   0.9999977636853660  -0.0013940553067130
         0.0002719847482266   0.0013936244652375   0.9999989919170660
...
SWEA Frames
   ...
   Z as shown on this diagram (see [6],[8], [13]):
   ...
   then by +140 degrees about Z (see [14]).

   Since SWEA changes its orientation with respect to the spacecraft
   when the boom is deployed, the MAVEN_SWEA frame is defined as a
   CK-based frame and its orientation is provided in a CK file.

   \begindata

      FRAME_MAVEN_SWEA              = -202130
      FRAME_-202130_NAME            = 'MAVEN_SWEA'
      FRAME_-202130_CLASS           = 3
...
APP Frames
   ...
   The following SC -> APP_BP misalignment DCM derived from October
   2014 APP/IUVS calibrations was provided in [16]:

      Gimbal baseplate to spacecraft:
      q_bp2sc =[-0.000981595772222 -0.003399337848236  0.001634848389496
                 0.999992404092490]; (scalar last)
      dcm_bp2sc =
         0.999971543545874   0.003276345493998   0.006795414533826
        -0.003262998391357   0.999992727480967  -0.001974291436236
        -0.006801833574895   0.001952061828210   0.999974961943867
      (named according to Vsc = dcm_bp2sc*Vbp)

   This matrix is incorporated in the column-major order in the
   MAVEN_APP_BP frame definition below.
   ...
   \begindata
      ...
      TKFRAME_-202501_SPEC            = 'MATRIX'
      TKFRAME_-202501_RELATIVE        = 'MAVEN_SPACECRAFT'
      TKFRAME_-202501_MATRIX          = (

         0.999971543545874  -0.003262998391357  -0.006801833574895
         0.003276345493998   0.999992727480967   0.001952061828210
         0.006795414533826  -0.001974291436236   0.999974961943867
...
IUVS Frames
   ...
   to co-align the MAVEN_IUVS_BASE frame with each of the FOV frames:
      ...
      FRAME_MAVEN_IUVS_LIMB         -167.85 about X (*)

      FRAME_MAVEN_IUVS_NADIR         -90.00 about X

   (*) -167.85 is based on [15]. The value from [6] used in the FK
       up to 11/05/14 was 167.75 degrees.
   ...
   Reference [10] provides the following "Final Flight Knowledge Data,
   Measured DCM, IUVS to ARF (IUVS in Columns relative to ARF)" (NOTE:
   the ARF frame used by the ATLO mechanical team is not the same as
   the APP frame. It is related to the APP frame as follows: +Zarf = +Xapp,
   +Xarf = +Yapp, +Yarf = +Zapp ):

         0.0006811951804074   0.9999995050399060   0.0007043195790375
         0.0004427444618388  -0.0007046097688996   0.9999996659425830
         0.9999996672120480  -0.0006808227932497  -0.0004432543901292

   This DCM is provided in the FK comments only for the record.

   The following APP -> IUVS_BASE misalignment DCM derived from October
   2014 APP/IUVS calibrations was provided in [16]:

      APP ideal to IUVS:
      q_app2iuvs =[-0.000042106202058 -0.001278614348199 -0.000083845824281
                    0.999999178170809];  (scalar last)
      dcm_app2iuvs =
         0.999996716230453  -0.000167583835560   0.002557233655651
         0.000167799185936   0.999999982393891  -0.000083997921960
        -0.002557219533934   0.000084426747856   0.999996726744833
      (named according to Viuvs = dcm_app2iuvs *Vapp)
   ...
   \begindata
      ...
      TKFRAME_-202510_SPEC            = 'MATRIX'
      TKFRAME_-202510_RELATIVE        = 'MAVEN_APP'
      TKFRAME_-202510_MATRIX          = (
      ...
      TKFRAME_-202511_ANGLES          = ( 167.85, 0.0, 0.0 )
...
STATIC Frames
   ...
   \begindata
      ...
      TKFRAME_-202520_ANGLES          = ( 0.0, 180.0, 0.0 )
...
NGIMS Frames
   ...
   Reference [10] provides the following "Final Flight Knowledge Data,
   Measured DCM, NGIMS to ARF (NGIMS in Columns relative to ARF)" (NOTE:
   the ARF frame used by the ATLO mechanical team is not the same as
   the APP frame. It is related to the APP frame as follows: +Zarf = +Xapp,
   +Xarf = +Yapp, +Yarf = +Zapp):

        -0.0005931154604816   0.9999998231269520  -0.0000442731977709
        -0.0001900106933984   0.0000441605064594   0.9999999809728930
         0.9999998060549750   0.0005931238615774   0.0001899844675115

   This DCM is provided in the FK comments only for the record.

   Reference [10] provides the following "Final Flight Knowledge Data,
   Measured DCM, NGIMS to IUVS (NGIMS in Columns relative to IUVS)":

         0.9999989851135120   0.0012743382759349   0.0006326986990127
        -0.0012738039447299   0.9999988932387770  -0.0007490122771062
        -0.0006336826769176   0.0007482170414141   0.9999995315215530

   This matrix is incorporated in the column-major order in the frame
   definition below.
   ...
   \begindata
      ...
      TKFRAME_-202530_SPEC            = 'MATRIX'
      TKFRAME_-202530_RELATIVE        = 'MAVEN_IUVS_BASE'
      TKFRAME_-202530_MATRIX          = (

         0.9999989851135120  -0.0012738039447299  -0.0006336826769176
         0.0012743382759349   0.9999988932387770   0.0007482170414141
         0.0006326986990127  -0.0007490122771062   0.9999995315215530
...
MAG Frames
   ...
   Reference [11] provides the following "Final Flight Knowledge Data,
   Measured DCM, +Y Mag (deployed) to SMRF (+Y MAG in Columns relative
   to SMRF)":

         0.9998989258688740  -0.0139227229591237   0.0028839221604874
         0.0140664580983611   0.9390992190336830  -0.3433581182582010
         0.0020721909247262   0.3433639765911090   0.9392001338532760


   and "Final Flight Knowledge Data, Measured DCM, -Y Mag (deployed) to
   SMRF (-Y MAG in Columns relative to SMRF)":

        -0.9999988795987810   0.0013574393811262   0.0006245863854926
        -0.0010752666664344  -0.9439748700253760   0.3300155899358680
         0.0010375700782834   0.3300145498755780   0.9439752758689820

   Based on the decision by the MAG PI in October 2014 these DCMs were
   not incorporated into the MAG frame definitions. Instead the
   definitions use the nominal MAG sensor alignments.
...
========================================================================
   ...
   -----------
      ...
      MAVEN_SC_BUS             -202000  MAVEN_SPACECRAFT, MAVEN_SPACECRAFT_BUS
   ...
   -----------------------------------------------
      ...
      MAVEN_EUV                -202110
      MAVEN_EUV-A              -202111  MAVEN_EUV1  
      MAVEN_EUV-B              -202112  MAVEN_EUV2
      MAVEN_EUV-C              -202113  MAVEN_EUV3  
   ...
   \begindata
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP1_FWD_A_ELEC'          )
      NAIF_BODY_CODE += ( -202121                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP1_FWD_B_ION'           )
      NAIF_BODY_CODE += ( -202122                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP1_AFT_B_ELEC'          )
      NAIF_BODY_CODE += ( -202123                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP1_AFT_A_ION'           )
      NAIF_BODY_CODE += ( -202124                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP1_FWD_A_ELEC_S'        )
      NAIF_BODY_CODE += ( -202161                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP1_FWD_B_ION_S'         )
      NAIF_BODY_CODE += ( -202162                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP1_AFT_B_ELEC_S'        )
      NAIF_BODY_CODE += ( -202163                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP1_AFT_A_ION_S'         )
      NAIF_BODY_CODE += ( -202164                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP2_FWD_A_ELEC'          )
      NAIF_BODY_CODE += ( -202126                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP2_FWD_B_ION'           )
      NAIF_BODY_CODE += ( -202127                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP2_AFT_B_ELEC'          )
      NAIF_BODY_CODE += ( -202128                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_SEP2_AFT_A_ION'           )
      NAIF_BODY_CODE += ( -202129                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP2_FWD_A_ELEC_S'        )
      NAIF_BODY_CODE += ( -202166                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP2_FWD_B_ION_S'         )
      NAIF_BODY_CODE += ( -202167                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP2_AFT_B_ELEC_S'        )
      NAIF_BODY_CODE += ( -202168                          )

      NAIF_BODY_NAME += ( 'MAVEN_SEP2_AFT_A_ION_S'         )
      NAIF_BODY_CODE += ( -202169                          )
      ...
      NAIF_BODY_NAME += ( 'MAVEN_STATIC_FRONT'             )
      NAIF_BODY_CODE += ( -202522                          )

      NAIF_BODY_NAME += ( 'MAVEN_STATIC_BACK'              )
      NAIF_BODY_CODE += ( -202523                          )
