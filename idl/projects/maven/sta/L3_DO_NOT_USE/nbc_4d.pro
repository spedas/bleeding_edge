;+
;FUNCTION:	nbc_4d(dat,ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins,MASS=ms,m_int=mi,q=q,mincnt=mincnt)
;INPUT:	
...
;LAST MODIFICATION:
;-
function nbc_4d,dat2,ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins,MASS=ms,m_int=mi,q=q,mincnt=mincnt,correct=correct,scale=scale,lpw=lpw
...
; including the ion suppression correction appears to make little difference because a shift of bins only includes empty low energy bins
	...
	if ndimen(data) eq 2 then begin
		...
		maxcnt3 = max(total(data-bkg,2),mind3)
;print,mind,mind3,energy[mind],energy[mind3]
...
if size(mvn_lpw_pot_c6_dat,/type) eq 8 and dat.data_name eq 'c6 32e64m' and not keyword_set(lpw) then begin
	...
	denergy_def = denergy
	nrg = reform(energy[*,0])	
	nrg_bnd = (nrg[0:30]+nrg[1:31])/2.
	nrg_bnd = [(2.*nrg_bnd[0]-nrg_bnd[1])>0.,[nrg_bnd],2.*nrg_bnd[30]-nrg_bnd[29]]
	denergy=(nrg_bnd[0:31]-nrg_bnd[1:32])#replicate(1.,64)
endif
