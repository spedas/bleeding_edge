;+
;FUNCTION:	dn_4d_f(dat,ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins,MASS=mass,m_int=mi,q=q,mincnt=mincnt)
;INPUT:	
...
;LAST MODIFICATION:
;-
function dn_4d_f,dat2,ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins,MASS=ms,m_int=mi,q=q,mincnt=mincnt,nn=nn,print=print,seed=seed
...
;	seed=total(tmp.data)/!pi
	;seed=total(tmp.data*(i+7))/!pi  ;CMF Removed this
