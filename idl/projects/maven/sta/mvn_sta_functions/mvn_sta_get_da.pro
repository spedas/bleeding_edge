...
;-
FUNCTION mvn_sta_get_da,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

common mvn_da,get_ind,all_dat

if n_elements(get_ind) eq 0 then begin
	if keyword_set(times) then return,0
	dat = 	{project_name:'MAVEN',valid:0}
	print,' ERROR - mvn da data not loaded'
	return,dat
...
;		nenergy: 		64, 							$
		energy: 		nrg, 							$
		nrate:			all_dat.nrate,						$
		rate_labels:		all_dat.rate_labels[r_ch],				$
		rate_channel:		r_ch,							$
