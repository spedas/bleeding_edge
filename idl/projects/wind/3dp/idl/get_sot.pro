...
function get_sot,t,add, times=tms, index=idx, chanmode=chanmode  ;, advance = adv
@wind_com.pro

dat = { sot_struct, $
   PROJECT_NAME:   'Wind 3D Plasma', $
   DATA_NAME:      'SST Open+Thick', $
   UNITS_NAME:     'Counts', $
   ...
   theta: fltarr(9, 16), $
   dtheta: fltarr(9, 16), $
   bins      :     replicate(1b,9,16), $
   dt        :     fltarr(9,16), $
   gf        :     fltarr(9,16), $
   bkgrate   :     fltarr(9,16), $
   deadtime  :     fltarr(9,16), $
   dvolume   :     fltarr(9,16), $
   ddata     :     replicate(!values.f_nan,9,16), $
...
;   num = call_external(wind_lib,'sot_to_idl')
   num = 100000
   options(0) = num
   times = dblarr(num)
   ok = call_external(wind_lib,'sot_to_idl',options,times)
   print,ok+1,'  SST Open+Thick time samples'
...
oldtime = dat.time
repeat begin
	ok = call_external(wind_lib,'sot_to_idl',options,time,dat)
