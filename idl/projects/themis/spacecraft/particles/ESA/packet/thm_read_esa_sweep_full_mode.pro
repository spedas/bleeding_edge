...
; The following is for LEO onlyA
...
e_energy=fltarr(32,8)
e_denergy=fltarr(32,8)
e_nenergy=intarr(8)
;e_cal=2.
;e_cal=795.6/419.
...
; Electron mode 3 - Full sweep, no linear, 15E Msph mode
	...
	e_energy(0:14,3)=(tmp(2*indgen(15)+1)+tmp(2*indgen(15)+2))/2.
	e_denergy(0:14,3)=e_cal(fm-1)*(1.*dac(3,2*indgen(15))+dac(0,2*indgen(15)+1)-dac(3,2*indgen(15)+2)-dac(0,2*indgen(15)+3))/2.
	e_nenergy(3)=15
...
; Electron mode 7 - Full sweep, no linear, 32E Msph mode, low E
  dac=thm_esa_energy_steps(xstart=16000,xslope=38,cstart=124,cslope=4,number=128,retrace=2,dblsweep=0)  
  e_energy(*,7)=e_cal(fm-1)*(1.*dac(0,*)+dac(1,*)+dac(2,*)+dac(3,*))/4.
  e_denergy(1:30,7)=e_cal(fm-1)*(1.*dac(3,0:29)+dac(0,1:30)-dac(3,1:30)-dac(0,2:31))/2.
  e_denergy(31,7)=e_cal(fm-1)*(1.*dac(3,30)+dac(0,31)-2.*dac(3,31))/2.
  e_nenergy(7)=32
