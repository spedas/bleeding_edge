;+
;
;FUNCTION:        ESC_IESA_GET_F4D
;
;PURPOSE:         Returns EESA-i Fine 4D (F4D) data stored in common blocks.
;
...
;
;-
FUNCTION esc_iesa_get_f4d, itime, blue=blue, gold=gold, index=ind, times=times, dynamic=dynamic;, array=array
  ...
  cvar = 'escp_iesa_f4d'
  dat  = SCOPE_VARFETCH(cvar.replace('p', p), common='esc_iesa_f4d_com')
  par  = SCOPE_VARFETCH(cvar.replace('p', p) + '_par', common='esc_iesa_f4d_com')
  ...
  IF nw GT 1 THEN BEGIN
     data.energy   = TRANSPOSE(par.energy[eidx, *, *, *], [1, 2, 3, 0])
     data.denergy  = TRANSPOSE(par.denergy[eidx, *, *, *], [1, 2, 3, 0])
     data.theta    = TRANSPOSE(par.theta[eidx, *, *, *], [1, 2, 3, 0])
     data.dtheta   = TRANSPOSE(par.dtheta[eidx, *, *, *], [1, 2, 3, 0])
     data.phi      = TRANSPOSE(par.phi[eidx, *, *, *], [1, 2, 3, 0])
     data.dphi     = TRANSPOSE(par.dphi[eidx, *, *, *], [1, 2, 3, 0])
     data.domega   = TRANSPOSE(par.domega[eidx, *, *, *], [1, 2, 3, 0])
