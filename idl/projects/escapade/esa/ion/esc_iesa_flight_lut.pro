...
FUNCTION esc_iesa_sci_luts_write_csv, table
   ...
   ;;printf, 1, '# ' + table.note
   printf, 1, '# '
   printf, 1, '# Main Science Product Dimensions'
   ...
   printf, 1, '# '
   printf, 1, '# Energy-Mass Product Dimensions'
   printf, 1, '# Mass:       '+ string(table.tbl_mas_rr_em,format='(I2)')
   printf, 1, '# Anode:      '+ string(table.tbl_ano_rr_em,format='(I2)')
   printf, 1, '# Deflection: '+ string(table.tbl_def_rr_em,format='(I2)')
   printf, 1, '# Energy:     '+ string(table.tbl_nrg_rr_em,format='(I2)')
   printf, 1, '# '
   printf, 1, '# '
   printf, 1, '# '
   printf, 1, '# '
   ...
   printf, 1, '#  '
   printf, 1, '#  '   
   printf, 1, '#    --- Fine Masses 0 ---     #'
   printf, 1, '#    30keV-2keV [Bins 0:16]    #'
   printf, 1, '# '+strjoin(replicate('-',30))
   printf, 1, '# Mass:   '+ string(table.nn_mas_fm,format='(I2)')
   printf, 1, '# Energy: '+ string(table.nn_nrg_fm,format='(I2)')
   printf, 1, '# '
   ...
   printf, 1, '# '
   printf, 1, '# ELUT - 64 Bin Array'
   FOR i=0, 7 DO printf, 1, format='(8I4)',table.nrg_lut_fm_1[8*i:8*i+7]
   printf, 1, '#  '
   printf, 1, '#  '
   printf, 1, '#  '
   printf, 1, '#    --- Fine Masses 1 ---     #'   
   printf, 1, '#    2keV-500eV [Bins 17:25]   #'
   printf, 1, '# '+strjoin(replicate('-',30))
   printf, 1, '# Mass:   '+ string(table.nn_mas_fm,format='(I2)')
   printf, 1, '# Energy: '+ string(table.nn_nrg_fm,format='(I2)')
   printf, 1, '# '
   ...
   printf, 1, '# '
   printf, 1, '# ELUT - 64 Bin Array'
   FOR i=0, 7 DO printf, 1, format='(8I4)',table.nrg_lut_fm_2[8*i:8*i+7]
   printf, 1, '#  '
   printf, 1, '#  '
   printf, 1, '#  '
   printf, 1, '#    --- Fine Masses 2 ---     #'      
   printf, 1, '#    500eV-1.5eV [Bins 26:63]  #'
   printf, 1, '# '+strjoin(replicate('-',30))
   printf, 1, '# Mass:   '+ string(table.nn_mas_fm,format='(I2)')
   printf, 1, '# Energy: '+ string(table.nn_nrg_fm,format='(I2)')
   printf, 1, '# '
   ...
   printf, 1, '# '
   printf, 1, '# ELUT - 64 Bin Array'
   FOR i=0, 7 DO printf, 1, format='(8I4)',table.nrg_lut_fm_3[8*i:8*i+7]
   printf, 1, '#  '
   printf, 1, '#  '
   printf, 1, '#  '   
...
PRO esc_iesa_flight_lut, tbl, write_csv=write_csv

   ;; --- Full 4D Product Dimension---
   nrg_rr = 64
   ano_rr = 16
   def_rr = 8
   mas_rr = 4

   ;; --- Energy-Mass Product Dimension ---
   nrg_rr_em = 64
   ano_rr_em = 1
   def_rr_em = 1
   mas_rr_em = 64
   ...
   ;; --- 2keV - 30keV [Bins  0:16] --- 
   ;; --- 500eV - 2keV [Bins 17:25] --- 
   ;; --- 1.5eV - 500eV [26:63] --- 
   
   ;; --- 2keV - 30keV [Bins  0:16] --- 
   nn_nrg_fm_1 = 17
   fm_1 = intarr(nn_mas_fm, nn_nrg_fm_1)    
   nrg_lut_fm_1 = [replicate(0,17), replicate(65,47)]

   ;; --- 500eV - 2keV [Bins 17:25] --- 
   nn_nrg_fm_2 = 9
   fm_2 = intarr(nn_mas_fm, nn_nrg_fm_2)    
   nrg_lut_fm_2 = [replicate(65,17), replicate(0,9), replicate(65,38)]

   ;; --- 1.5eV - 500eV [26:63] --- 
   nn_nrg_fm_3 = 38
   fm_3 = intarr(nn_mas_fm, nn_nrg_fm_3)    
   nrg_lut_fm_3 = [replicate(65,26), replicate(0,38)]
   ...
   tbl = { $
         ...
         tbl_mas_rr_em:mas_rr_em, tbl_ano_rr_em:ano_rr_em, tbl_def_rr_em:def_rr_em, tbl_nrg_rr_em:nrg_rr_em,$

         nn_mas_fm:nn_mas_fm, nn_nrg_fm:nn_nrg_fm, $
         mas_lut_fm:mas_lut_fm, nrg_lut_fm:nrg_lut_fm, $
         nrg_lut_fm_1:nrg_lut_fm_1, nrg_lut_fm_2:nrg_lut_fm_2, nrg_lut_fm_3:nrg_lut_fm_3, $
   ...
   IF keyword_set(write_csv) THEN  tmp = esc_iesa_sci_luts_write_csv(tbl)
