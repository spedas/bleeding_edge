...
PRO esc_iesa_load, itime, product=product, data=data, verbose=verbose, level=level, blue=blue, gold=gold, files=afile, $
                   ipath=ipath, source=source, no_server=no_server, prelaunch=prelaunch, commissioning=commissioning, clear=clear, all=all

  COMMON esc_iesa_fe_com,   escb_iesa_fe,  escg_iesa_fe,  escb_iesa_fe_par, escg_iesa_fe_par  ; Fine Energies (0x134)
  COMMON esc_iesa_fm_com,   escb_iesa_fm,  escg_iesa_fm,  escb_iesa_fm_par, escg_iesa_fm_par  ; Fine Masses   (0x147)
  ...
  FOR i=0, N_ELEMENTS(probes)-1 DO FOR j=0, N_ELEMENTS(prod)-1 DO BEGIN
     ...
     IF ~undefined(param) THEN undefine, EXECUTE('esc' + probes[i].substring(0, 0) + '_iesa_' + prod[j] + '_par = param')
  ENDFOR
