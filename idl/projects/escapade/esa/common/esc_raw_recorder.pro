...
;    Roberto Livi - April 2023
;
; $LastChangedBy: rlivi04 $
; $LastChangedDate: 2023-04-25 10:00:18 -0700 (Tue, 25 Apr 2023) $
; $LastChangedRevision: 31794 $
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/trunk/projects/escapade/esa/common/esc_raw_recorder.pro $
;
;-

PRO esc_raw_recorder_event, ev   ; recorder
  ;   on_error,1
  ...
  CASE ev.id OF                         ;  Timed events
        ...
        if info.file_timeres ne 0 then begin
          if info.time_received ge info.next_filechange then begin
            ...
            if info.dfp then begin
              esc_raw_recorder_event,{top: ev.top, id:wids.dest_button}   ; close old file  - possible error that dfp might change!
              esc_raw_recorder_event,{top: ev.top, id:wids.dest_button}   ; open  new file
            endif
          endif
          widget_control, ev.top, get_uvalue= info   ; get all widget ID's
    ...
    wids.dest_flush: begin
      esc_raw_recorder_event,{top: ev.top, id:wids.dest_button}   ; close old file
      esc_raw_recorder_event,{top: ev.top, id:wids.dest_button}   ; open  new file
    end
...
PRO esc_raw_recorder,base,title=title,ids=ids,host=host,port=port,destination=destination,exec_proc=exec_proc, $
  ...
  if ~(keyword_set(base) && widget_info(base,/managed) ) then begin
    ...
    XMANAGER, 'esc_raw_recorder', ids.base,/no_block
  ...
  if n_elements(set_output)  eq 1 && (keyword_set(info.dfp) ne keyword_set(set_output )) then esc_raw_recorder_event, { id:ids.dest_button, top:ids.base }
  if n_elements(set_connect) eq 1 && (keyword_set(info.hfp) ne keyword_set(set_connect)) then esc_raw_recorder_event, { id:ids.host_button, top:ids.base }
  if n_elements(set_procbutton) eq 1 then begin
    widget_control,ids.proc_button,set_button=set_procbutton
    esc_raw_recorder_event, { top:ids.base, id:ids.proc_button, select: keyword_set(set_procbutton) }
  endif
