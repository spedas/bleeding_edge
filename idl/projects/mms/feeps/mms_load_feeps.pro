...
;         quality_flag: Value can range from 2-4. Determines which eyes to use to calculate omnidirectional flux at a given time based on how many sectors were masked for sunlight.
;                       For BRST or RAW:
...
;       get_err:  Gets the error in the omnidirectional l2 intensity.  0 value is off, gt 0 sets it on. User MUST run mms_load_feeps for the l1b counts data FIRST to use this function. 
;                 (Note: the FEEPS l1b data needed for this switch is restricted to MMS team members at present.)
;
...
pro mms_load_feeps, trange = trange, probes = probes, datatype = datatype, $
                  ...
                  quality_flag=quality_flag, get_err=get_err
    ...
    if undefined(quality_flag) then quality_flag = 3. ;added 10-03-2025 by Christine Gabrielse to avoid  using poor quality data when creating omnidirectional fluxes.
      ...
      for datatype_idx = 0, n_elements(datatype_in)-1 do begin
        ...
        for data_units_idx = 0, n_elements(data_units)-1 do begin
          ...
          mms_feeps_omni, this_probe, datatype = this_datatype, tplotnames = tplotnames, data_units = data_units[data_units_idx], $
              data_rate = data_rate_in, suffix=suffix, level = level_in, sensor_eyes = eyes, quality_flag=quality_flag,get_err=get_err
