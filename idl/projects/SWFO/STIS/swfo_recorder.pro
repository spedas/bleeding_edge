...
PRO swfo_recorder_event, ev   ; recorder
  ;   on_error,1
  ...
  CASE ev.id OF                         ;  Timed events
    wids.base:  begin
      ...
      if info.hfp gt 0 then begin
        ...
        if info.file_timeres ne 0 then begin
          if info.time_received ge info.next_filechange then begin
            ...
            if info.dfp then begin
              swfo_recorder_event,{top: ev.top, id:wids.dest_button}   ; close old file  - possible error that dfp might change!
              swfo_recorder_event,{top: ev.top, id:wids.dest_button}   ; open  new file
            endif
          endif
          widget_control, ev.top, get_uvalue= info   ; get all widget ID's
        ...
        if 1 then begin
          swfo_gsemsg_lun_read,info.hfp,info.dfp,info=info
          widget_control,wids.base,set_uvalue=info
          msg = info.msg
    ...
    wids.dest_flush: begin
      swfo_recorder_event,{top: ev.top, id:wids.dest_button}   ; close old file
      swfo_recorder_event,{top: ev.top, id:wids.dest_button}   ; open  new file
    end
...
PRO swfo_recorder,base,title=title,ids=ids,host=host,port=port,destination=destination,exec_proc=exec_proc, $
  ...
  if ~(keyword_set(base) && widget_info(base,/managed) ) then begin
    ...
    if not keyword_set(title) then title = 'SWFO GSE Recorder'
    port=strtrim(port,2)
    ...
    XMANAGER, 'swfo_recorder', ids.base,/no_block
  ...
  if n_elements(set_output)  eq 1 && (keyword_set(info.dfp) ne keyword_set(set_output )) then swfo_recorder_event, { id:ids.dest_button, top:ids.base }
  if n_elements(set_connect) eq 1 && (keyword_set(info.hfp) ne keyword_set(set_connect)) then swfo_recorder_event, { id:ids.host_button, top:ids.base }
  if n_elements(set_procbutton) eq 1 then begin
    widget_control,ids.proc_button,set_button=set_procbutton
    swfo_recorder_event, { top:ids.base, id:ids.proc_button, select: keyword_set(set_procbutton) }
  endif
