...
;-
FUNCTION get_tha_peec,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

dat=get_tha_peef(time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times)

if keyword_set(times) then return,dat
if dat.nenergy eq 32 then return,dat

dat2=get_tha_peer(dat.time)
abs_dt = abs(dat.time-dat2.time)

if abs_dt gt 0.1 then begin
	if abs_dt lt 2. then begin
		dprint, 'significant spin period time error',time_string(dat.time)
	endif else begin
		dprint, 'missing peer data',time_string(dat.time)
		return,dat
	endelse
endif
