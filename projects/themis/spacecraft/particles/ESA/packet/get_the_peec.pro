...
;-
FUNCTION get_the_peec,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

dat=get_the_peef(time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times)

if keyword_set(times) then return,dat
if dat.nenergy eq 32 then return,dat

dat2=get_the_peer(dat.time)
abs_dt = abs(dat.time-dat2.time)

if abs_dt gt 0.2 then begin
	if abs_dt lt 2. then begin
		dprint, 'get_the_peec: significant spin period time error  ',abs_dt,'  ',time_string(dat.time,/msec),'  ',time_string(dat2.time,/msec)
	endif else begin
		dprint, 'get_the_peec: missing peer data',time_string(dat.time)
		return,dat
	endelse
endif

if dat2.nenergy eq 32 and dat2.nbins eq 1 then begin
...
;		domega: 		domega,  						$
	...
	fr0 = dat2.bkg[0]/(dat2.bkg[1]+dat2.bkg[2]+1.e-6)
	fr1 = dat2.bkg[ind1]/(dat2.bkg[ind1]+dat2.bkg[ind2]+1.e-6)
	fr2 = dat2.bkg[ind2]/(dat2.bkg[ind1]+dat2.bkg[ind2]+1.e-6)
	fr3 = dat2.bkg[31]/(dat2.bkg[30]+dat2.bkg[29]+1.e-6)
	...
	fr0 = dat2.data[0]/(dat2.data[1]+dat2.data[2]+1.e-6)
	fr1 = dat2.data[ind1]/(dat2.data[ind1]+dat2.data[ind2]+1.e-6)
	fr2 = dat2.data[ind2]/(dat2.data[ind1]+dat2.data[ind2]+1.e-6)
	fr3 = dat2.data[31]/(dat2.data[30]+dat2.data[29]+1.e-6)
