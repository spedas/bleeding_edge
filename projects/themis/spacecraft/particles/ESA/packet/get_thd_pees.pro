...
;-
FUNCTION get_thd_pees,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

dat2=get_thd_peer(time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times)

if keyword_set(times) then return,dat2

dat=get_thd_peef(dat2.time)
abs_dt = abs(dat.time-dat2.time)

if abs_dt gt 0.2 then begin
	if abs_dt lt 2. then begin
		dprint, 'get_thd_pees: significant spin period time error  ',abs_dt,'  ',time_string(dat.time,/msec),'  ',time_string(dat2.time,/msec)
	endif else begin
;		print,'get_thd_pees: missing peer data',time_string(dat.time)
		return,dat2
	endelse
endif
