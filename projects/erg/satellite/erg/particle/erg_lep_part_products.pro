;+
; !!!!!  CAUTION   !!!!!
;This is a higly experimental version of the part_products libraries
;for LEP-e and LEP-i data of the ERG (Arase) satellite. This includes
;work in progress, experimental changes, and transitional functions
;that might or might not be present in future experimental
;releases and the official releases of the softwar. The
...
;               erg_lep?_l2_3dflux_F?DU is acceptable.
;  species: a string of particle species name. currently the following
;          strings are acceptable: 'proton', 'heplus', 'oplus' 
...
pro erg_lep_part_products, $
  ...
  instnm = (strsplit(/ext, in_tvarname, '_'))[1] ;; should be 'lepe' or 'lepi'
  lvl = (strsplit(/ext, in_tvarname, '_'))[2] ;; 'l2'
  datnm = (strsplit(/ext, in_tvarname, '_'))[3] ;; '3dflux'

  ;; Check if a tplot variable given is acceptable
  if ( ~strcmp(instnm, 'lepe') and ~strcmp(instnm, 'lepi') ) $
     or ~strcmp(datnm, '3dflux') then begin
    dprint, dlevel=0, 'The tplot variable given as an argument is not valid!'
    dprint, 'varname: '+in_tvarname
    return
  endif
  ...
  case instnm of
    'lepe': begin
      alldist = erg_lepe_get_dist(in_tvarname, /structure, species=species, $
                                units=input_units)
    end
    'lepi': begin
      alldist = erg_lepi_get_dist(in_tvarname, /structure, species=species, $
                                units=input_units)
    end
  endcase
  ...
  for i=0L, n_elements(time_idx)-1 do begin
    ...
    case instnm of
      ...
      'lepi': begin
        ...
        dprint, 'ERROR: Cannot find "lepe" or "lepi" in the given tplot variable name: '+in_tvarname
        return
      endelse
    endcase
    ...
    if in_set(outputs_lc, 'moments') || in_set(outputs_lc, 'fac_moments') then begin
      erg_pgs_clean_data, dist, output=clean_data, units=units_lc, $
        magf=magvec, /for_moments ;; invalid values are zero-padded. 
    endif else begin
      erg_pgs_clean_data, dist, output=clean_data, units=units_lc, $
        magf=magvec
    endelse
    ...
    if in_set(outputs_lc, 'energy') then begin
      erg_pgs_make_e_spec, clean_data, spec=en_spec, yaxis=en_y
    endif
    ...
    if in_set(outputs_lc, 'fac_energy') then begin
      erg_pgs_make_e_spec, clean_data, spec=fac_en_spec,  yaxis=fac_en_y
    endif
    ;;;;;;help, clean_data
  ...
  made_et_spec = ( ~undefined(en_spec) or ~undefined(fac_en_spec) )
  ...
  if strmatch(tplot_prefix, 'erg_lepe_*')  and made_et_spec  then begin
    if debug then dprint, ' Sorting a data array by energy...'
    
    get_data, tplot_prefix+'energy'+suffix, data=d, dl=dl, lim=lim
    
    for ii=0L, n_elements(d.x)-1 do begin
      if ndimen(d.v) eq 2 then v = reform(d.v[ii, *]) else v = d.v
      id = sort( v )
      new_v = v[id] & new_y = reform( d.y[ii, [id]] )
      d.y[ii, *] = transpose(new_y)
      if ndimen(d.v) eq 2 then d.v[ii, *] = transpose(new_v) else d.v = new_v
    endfor

    store_data, tplot_prefix+'energy'+suffix, data=d, dl=dl, lim=lim
  endif
