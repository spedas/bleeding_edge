...
function get_elb,t,add, times=tms, index=idx  ;, advance = adv
@wind_com.pro

dat = { elb_struct, $
   PROJECT_NAME:   'Wind 3D Plasma', $
   DATA_NAME:      'Eesa Low Burst', $
   UNITS_NAME:     'Counts', $
...
;   num = call_external(wind_lib,'elb_to_idl')
   ...
   print,ok+1,'  Eesa low burst time samples'
...
oldtime=dat.time
repeat begin
	ok = call_external(wind_lib,'elb_to_idl',options,time,dat)
