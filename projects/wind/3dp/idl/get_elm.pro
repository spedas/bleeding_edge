...
function get_elm,t,add, times=tms, index=idx  ;, advance = adv
@wind_com.pro

dat = { elb_struct2, $
   PROJECT_NAME:   'Wind 3D Plasma', $
   DATA_NAME:      'Eesa Low Merge', $
   UNITS_NAME:     'Counts', $
...
;   num = call_external(wind_lib,'elm_to_idl')
   num = 10000
   options(0) = num
   times = dblarr(num)
   ok = call_external(wind_lib,'elm_to_idl',options,times)
   print,ok+1,'  Eesa low merge time samples'
...
oldtime=dat.time
repeat begin
	ok = call_external(wind_lib,'elm_to_idl',options,time,dat)
...
str_element,retdata,'CHARGE',-1d,/add
