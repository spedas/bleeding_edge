...
;-
FUNCTION mvn_sta_get_d4,time,START=st,EN=en,ADVANCE=adv,RETREAT=ret,index=ind,calib=calib,times=times

common mvn_d4,get_ind,all_dat

if n_elements(get_ind) eq 0 then begin
	if keyword_set(times) then return,0
	dat = 	{project_name:'MAVEN',valid:0}
	print,' ERROR - mvn d4 data not loaded'
	return,dat
...
dat = 		{project_name:		all_dat.project_name,		$
	...
	nbins   = all_dat.nbins
	nmass   = all_dat.nmass
	gf2	= reform(reform(all_dat.gf[swp_ind,*,*,att_ind],nenergy*nbins)#replicate(1.,nmass),nenergy,nbins,nmass)
...
;		header:			all_dat.header[ind],			$
		...
		nenergy: 		nenergy, 						$
		energy: 		reform(all_dat.energy[swp_ind,*,*,*],1,nbins,nmass), 	$
		denergy: 		reform(all_dat.denergy[swp_ind,*,*,*],1,nbins,nmass),  	$
		...
		theta: 			reform(all_dat.theta[swp_ind,*,*,*],1,nbins,nmass),  	$
		dtheta: 		reform(all_dat.dtheta[swp_ind,*,*,*],1,nbins,nmass),  	$
		phi: 			reform(all_dat.phi[swp_ind,*,*,*],1,nbins,nmass),  	$
		dphi: 			reform(all_dat.dphi[swp_ind,*,*,*],1,nbins,nmass),	$
		domega: 		reform(all_dat.domega[swp_ind,*,*,*],1,nbins,nmass),  	$

		gf: 			gf2,							$
		eff: 			reform(all_dat.eff[eff_ind,*,*,*],1,nbins,nmass),	$
		...
		nmass:			nmass,							$
		mass: 			all_dat.mass, 						$
		mass_arr: 		reform(all_dat.mass_arr[swp_ind,*,*,*],1,nbins,nmass), 	$
		tof_arr: 		reform(all_dat.tof_arr[mlut_ind,*,*,*],1,nbins,nmass), 	$
		twt_arr: 		reform(all_dat.twt_arr[mlut_ind,*,*,*],1,nbins,nmass), 	$
		...
		bkg:	 		reform(all_dat.bkg[ind,*,*,*],1,nbins,nmass),		$
		dead:	 		reform(all_dat.dead[ind,*,*,*],1,nbins,nmass),		$
		cnts:	 		reform(all_dat.data[ind,*,*,*],1,nbins,nmass),		$

		data: 			reform(all_dat.data[ind,*,*,*],1,nbins,nmass)}
