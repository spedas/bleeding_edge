...
;   $LastChangedDate: 2018-03-29 16:27:07 -0700 (Thu, 29 Mar 2018) $
;   $LastChangedRevision: 24970 $
...
pro mvn_spc_fov_blockage, trange=trange,$
   ...
   ;;------------------------------
   ;;Inner Gimbal
   g1_gim_loc=inst.g1_gim_loc
   ;;Outer Gimbal
   g2_gim_loc=inst.g2_gim_loc
   ;;STATIC
   sta_loc=inst.sta_loc
   ;;SWEA
   swe_loc=inst.swe_loc
   ;;SWIA
   swi_loc=inst.swi_loc
   ;;SEP +X/+
   sep1_loc=inst.sep_py_loc
   ;;SEP +X/-Y
   sep2_loc=inst.sep_my_loc
   ...
   if keyword_set(static) then begin
      ...
      IF SIZE(mvn_sta_fov_block_time, /type) EQ 0 THEN BEGIN
         ...
         ;;
         ;; New
         mvn_sta_fov_block_qrot1 = spice_body_att('MAVEN_STATIC', 'MAVEN_APP',        mvn_sta_fov_block_time, $
                                                  /quaternion, check_objects='MAVEN_SPACECRAFT')
         mvn_sta_fov_block_qrot2 = spice_body_att('MAVEN_APP',    'MAVEN_APP_OG',     mvn_sta_fov_block_time, $
                                                  /quaternion, check_objects='MAVEN_SPACECRAFT')
         mvn_sta_fov_block_qrot3 = spice_body_att('MAVEN_APP_OG', 'MAVEN_APP_IG',     mvn_sta_fov_block_time, $
                                                  /quaternion, check_objects='MAVEN_SPACECRAFT')
         mvn_sta_fov_block_qrot4 = spice_body_att('MAVEN_APP_IG', 'MAVEN_APP_BP',     mvn_sta_fov_block_time, $
                                                  /quaternion, check_objects='MAVEN_SPACECRAFT')
         mvn_sta_fov_block_qrot5 = spice_body_att('MAVEN_APP_BP', 'MAVEN_SPACECRAFT', mvn_sta_fov_block_time, $
                                                  /quaternion, check_objects='MAVEN_SPACECRAFT')
      ...
      IF SIZE(mvn_sta_fov_block_time, /type) NE 0 THEN BEGIN
         ...
         ;; New
         ;; Rotation of Outer Gimbal (Closer to APP)
         ...
         inst_loc = ((inst_loc-g2_gim_loc) ## g2_mm) + g2_gim_loc
         a11 = ((inst.apa_body-foc) ## g2_mm) + foc
         a12 = ((inst.apb_body-foc) ## g2_mm) + foc
         a13 = ((inst.sba_body-foc) ## g2_mm) + foc
         a14 = ((inst.sbb_body-foc) ## g2_mm) + foc

         
         ;; Rotation of Inner Gimbal (Closer to S/C, Y Axis of rotation)
         qrot1 =  REFORM(mvn_sta_fov_block_qrot4[*, idx])
         tmp1 = quaternion_rotation([0.0,1.0,1.0], qrot1, /last_ind)
         th1 = -1.*atan(tmp1[0],tmp1[2])
         g1_mm = [[cos(th1),        0, sin(th1)],$
                  [       0,        1,        0],$
                  [-sin(th1),       0, cos(th1)]]         
         foc = rebin(g1_gim_loc,3,8)
         inst_loc = ((inst_loc-g1_gim_loc) ## g1_mm) + g1_gim_loc
         vertex[*,*,10] = ((inst.gi2_body-foc) ## g1_mm) + foc           
         vertex[*,*,11] = ((a11-foc) ## g1_mm) + foc
         vertex[*,*,12] = ((a12-foc) ## g1_mm) + foc
         vertex[*,*,13] = ((a13-foc) ## g1_mm) + foc
         vertex[*,*,14] = ((a14-foc) ## g1_mm) + foc
   ...
   x1   = [-8000,8000]
   y1   = [-8000,8000]
   pos0 = [0.1,0.10,0.9,0.90]
   pos1 = [0.1,0.65,0.9,0.90]
   pos2 = [0.1,0.40,0.9,0.65]
   pos3 = [0.1,0.15,0.9,0.40]
   ...
   for iobj=0, inst.n3-1 do begin
      ...
      for i=0, inst.n2-1 do begin           
         ...
         plot, box[0,indd],box[1,indd],$
               ...
               ystyle=1,$
               position=pos1
         oplot, replicate(bck_inst_loc[0],2), $
                replicate(bck_inst_loc[1],2), $
                psym=1, thick=4, color=250
         oplot, replicate(sta_loc[0],2), $
                replicate(sta_loc[1],2), $
                psym=1, thick=4, color=50
         plot, box[1,indd],box[2,indd],$
               ...
               ystyle=1,$
               position=pos2
         oplot, replicate(bck_inst_loc[1],2), $
                replicate(bck_inst_loc[2],2), $
                psym=1, thick=4, color=250
         oplot, replicate(sta_loc[1],2), $
                replicate(sta_loc[2],2), $
                psym=1, thick=4, color=50
