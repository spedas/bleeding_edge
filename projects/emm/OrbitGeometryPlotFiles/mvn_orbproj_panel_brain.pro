...
;;; Get the crustal fields
  if keyword_set(crustalfields) eq 1 then begin
     ...
     restore, '/home/rlillis/work/mgs/magdata/old_magfiles/br_360x180_pc.sav'
...
;;; Add the overlay image
   if keyword_set(crustalfields) or keyword_set (overlay) then begin
      ...
      if keepcnt gt 0 then begin
         if keyword_set (overlay) then loadct2, overlay.color_table else $
            loadct2, 70, /reverse
         ;for k = 0, keepcnt-1 do begin & $
          ;  oplot, [0,xdat[keep [k]]], [0,ydat [keep [k]]], $
           ;                             color = color [keep[k]], psy = 4, symsize = 0.2& $
                   ;wait, 0.0001 & $
            ;       endfor
...
; if we want to plot the nightside
