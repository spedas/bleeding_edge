...
;
;-
function je_2d_new,dat2,ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins
...
;eflux2dz = total(data*denergy*energy^2*domega*cos(theta))*(mass)^(-2.)*(2.)*(1.e5)*1.6e-12
eflux2dz = total(data*denergy*energy^2*domega)*(mass)^(-2.)*(2.)*(1.e5)*1.6e-12
