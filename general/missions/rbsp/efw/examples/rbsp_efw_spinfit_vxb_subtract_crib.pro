...
;   Note: if you want to use another boom pair combination, use
;   rbsp_efw_create_esvy_uvw_from_vsvy.pro
...
pro rbsp_efw_spinfit_vxb_subtract_crib,probe,no_spice_load=no_spice_load,$
  noplot=noplot,ql=ql,qa=qa,hiresl3=hiresl3,level=level,boom_pair=pair
...
;-----------------------------------------
;Spinfit quicklook data and rotate to MGSE
;-----------------------------------------
...
;-----------------------------------------
;Spinfit L2 data and rotate to MGSE
;-----------------------------------------
  ...
  if level eq 'l2' then begin
    ...
    if cnt eq 1 then time_clip,rbspx+'_emfisis_l2_uvw_Mag',t0,t1,$
    replace=1,error=error
    ttst = tnames(rbspx+'_emfisis_l2_uvw_Magnitude',cnt)
    if cnt eq 1 then time_clip,rbspx+'_emfisis_l2_uvw_Magnitude',t0,t1,$
    replace=1,error=error
...
;-----------------------------------------
;Spinfit L3 data and rotate to MGSE
;-----------------------------------------
  ...
  if level eq 'l3' then begin
    ...
    if cnt eq 1 then time_clip,rbspx+'_emfisis_l3_'+type+'_gse_Mag',t0,t1,$
    replace=1,error=error
    ttst = tnames(rbspx+'_emfisis_l3_'+type+'_gse_Magnitude',cnt)
    if cnt eq 1 then time_clip,rbspx+'_emfisis_l3_'+type+'_gse_Magnitude',t0,t1,$
    replace=1,error=error
  ...
  dif_data,rbspx+'_state_vel_mgse',rbspx+'_state_vel_coro_mgse',$
    newname='vel_total'

  ;Subtract off VxB field
  ...
  store_data,['Esvy_mgse_vxb_magnitude','Esvy_mgse_vxb_removed','bfield_data_gei',$
